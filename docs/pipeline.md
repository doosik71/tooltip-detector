...
창은 위에서 아래로 세 영역으로 구성된다.

1. **Pipeline Steps**: 6개 단계 목록. 각 단계는 상태 아이콘, 단계 이름, 진행 상세, 설명, 그리고 `Run`(또는 6단계는 `Launch`) 버튼으로 구성된다.
2. **컨트롤 바**: `Refresh`, `Stop`, `Clear Log` 버튼.
3. **Log**: 실행 중인 단계의 출력이 실시간으로 표시되는 검은 배경의 로그 창.
...
각 단계의 상태는 `data/`, `temp/` 아래 파일 개수를 세어 자동 판정한다.
...
- **1. Generate Progressive**
  - 입력: `data/video`의 비디오 수, 출력: `data/progressive`의 비디오 수.
  - 입력이 0이면 `waiting`, 출력 ≥ 입력이면 `done`, 일부면 `partial`, 0이면 `pending`.
- **2. Generate Dataset**
  - `data/dataset/images/{train,val,test}`에 이미지가 하나라도 있으면 `done`, 없으면 `pending`.
- **3. Download Model**
  - `temp/models/model.pt`가 있으면 `done`, 없으면 `pending`.
- **4. Generate Segmentation**
  - 입력: 이미지 수, 출력: `data/dataset/segmentation/...`의 mask 수.
  - mask가 0이면 `pending`, mask ≥ 이미지면 `done`, 그 외 `partial`.
- **5. Generate Annotation**
  - 입력: mask 수, 출력: `data/dataset/annotation/...`의 JSON 수.
  - JSON이 0이면 `pending`, JSON ≥ mask면 `done`, 그 외 `partial`.
- **6. Annotation Editor**
  - annotation JSON이 하나라도 있으면 `ready`, 없으면 `waiting`(5단계 먼저 실행).
...
이 GUI는 각 단계를 **인자 없이** 실행한다. 따라서 모든 단계는 각 스크립트의 **기본 경로와 기본 옵션**으로만 동작한다.
...
```bash
uv run python -m scripts.generate_segmentation --overwrite
...
- `_step_progress(step_id)`
  - 단계별 `(status, detail)`을 계산한다.
- `_STATUS_UNICODE` / `_STATUS_ASCII`
  - 상태별 (아이콘, 색) 매핑. 심볼 폰트 유무에 따라 선택된다.
- `PipelineApp`
  - GUI 본체. 아래 메서드로 구성된다.
    - `__init__(root)`: 폰트/상태 스타일/큐 초기화, UI 빌드, 첫 새로고침, 폴링 시작.
    - `_build_ui()`: 단계 행, 컨트롤 바, 로그 창을 생성한다.
    - `_refresh()`: 각 단계 상태를 다시 계산해 표시한다.
    - `_set_running(idx)`: 실행 중 단계를 표시하고 버튼 상태를 토글한다.
    - `_run(idx)`: 해당 단계를 실행한다(편집기는 별도 창, 나머지는 작업 스레드).
...
`_run(idx)`는 `STEPS[idx]["editor"]` 값으로 분기한다.

- **편집기(editor=True)**: `subprocess.Popen([str(script)], cwd=ROOT)`로 별도 창을 띄우고, 출력은 캡처하지 않는다. 곧바로 큐에 `None`(완료 신호)을 넣어 GUI 잠금을 해제한다.
- **일반 단계(editor=False)**: 데몬 스레드에서 `subprocess.Popen(...)`을 `stdout=PIPE, stderr=STDOUT, text=True, bufsize=1`로 실행하고, 출력 라인을 하나씩 `_log_queue`에 넣는다. 프로세스가 끝나면 종료 코드에 따라 `✓ done`/`✗ error` 메시지와 완료 신호(`None`)를 큐에 넣는다.
...
각 단계 상태는 별도 메타 파일 없이 **출력 파일 개수**로 추정한다. 입력 대비 출력 비율로 `done`/`partial`/`pending`을 구분하고, 선행 입력이 없으면 `waiting`을 반환한다. 이는 가볍고 단순하지만, "건너뛴 것"과 "실제로 처리한 것"을 구분하지는 못한다(파일 존재 여부만 본다).
...
- 각 단계를 인자 없이 실행하므로, `--overwrite`/`--device`/경로 변경 등 CLI 옵션을 GUI에서 줄 수 없다.
...
for step_id in ("generate_dataset", "generate_segmentation", "generate_annotation"):
    status, detail = _step_progress(step_id)
