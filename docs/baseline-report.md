# 복강경 수술 영상에서의 객체 탐지를 이용한 수술도구 및 팁 탐지

## 실험결과보고서: CLAD-Net, YOLOv8s, YOLO26 비교와 참조 구현 대조

이 보고서는 [`baseline/`](../baseline/) 아래 다섯 서브 프로젝트의 실험을 종합한다.
셋은 세 아키텍처를 `ultralytics` 없이 PyTorch로 재구현해 **스크래치로 학습한 회차**이고,
둘은 그중 두 아키텍처를 **`ultralytics` 참조 구현으로, COCO 사전학습에서 파인튜닝한 회차**다.

| 축                    | 서브 프로젝트                                                                                                                                                                                             | 초기 가중치   |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 재구현 (본 비교의 축) | [cladnet](../baseline/cladnet/docs/experimental-results.md) · [yolov8sclone](../baseline/yolov8sclone/docs/experimental-results.md) · [yolo26clone](../baseline/yolo26clone/docs/experimental-results.md) | 무작위        |
| 참조 구현 (대조군)    | [yolov8s](../baseline/yolov8s/README.md) · [yolo26](../baseline/yolo26/docs/experimental-results.md)                                                                                                      | COCO 사전학습 |

여기서는 **세 아키텍처의 비교**와 **성능 차이의 원인**에 집중하고, 참조 구현 두 회차는
그 비교가 구현체와 초기 가중치를 바꿔도 유지되는지를 확인하는 대조군으로 쓴다(§4.4).

인용한 수치는 모두 `baseline/<name>/data/` 아래 산출물에서 재계산된다. 그림 4〜12는
[`baseline-report-graph.ipynb`](../notebook/baseline-report-graph.ipynb)가
같은 산출물에서 다시 그리고, 구조 도해인 그림 1〜3은 각 서브 프로젝트의
`docs/model-architecture.svg`에서 가져왔다.

**이름 규칙.** `CLAD-Net`·`YOLOv8s`·`YOLO26`은 별도로 밝히지 않는 한 **본 저장소의 자체
구현**을 가리킨다. 참조 구현 회차는 `YOLOv8s(참조)`·`YOLO26(참조)`로 적는다.

## 초록 (Abstract)

복강경 수술 영상에서 수술도구의 위치와 그 **끝점(tip)** 을 실시간으로 얻는 문제를,
객체 탐지 방식으로 접근한 세 방법론을 같은 조건에서 비교했다. 내시경 수술도구 탐지를
위해 제안된 전용 아키텍처 **CLAD-Net** [1], 범용 탐지기 **YOLOv8s** [2], 그리고 NMS를
없앤 최신 세대 **YOLO26** [14] 이다.
**세 모델 모두 `ultralytics` 등 외부 탐지 프레임워크 없이 PyTorch로 직접 구현했고, 사전학습
가중치 없이 처음부터 학습했다.** CLAD-Net은 저자 공개 코드가 없어 논문 본문과 도해만으로
재구성했고, YOLO26은 참조 구현의 출력·손실과 비트 단위로 일치함을 확인했다(§2.3).
여기에 **같은 두 아키텍처를 `ultralytics` 참조 구현으로, COCO 사전학습에서 학습한 두 회차**를
대조군으로 더해 다섯 회차를 같은 test 세트·같은 매칭 규칙으로 채점했다.

팁이라는 **점 어노테이션**을 32 × 32 px 상자로 바꾸어 `tool`·`tip` 2클래스 탐지 문제로
정식화함으로써, 하나의 탐지기가 도구 상자와 팁 좌표를 동시에 산출하도록 했다. 이로써 탐지
지표(AP)와 이 프로젝트의 팁 지표(Hit-rate @ N px)를 같은 모델에서 함께 측정할 수 있고,
기존 세그멘테이션 기반 접근 [3] 과 동일한 test 프레임·동일한 매칭 규칙으로 비교할 수 있다.

동일 조건(150 에포크, `--frame-stride 5`, 팁 32 px, 640 × 640 입력)에서 **두 범용 탐지기가
전용 아키텍처를 앞섰다.** cholec80 test mAP@0.5는 YOLOv8s 0.751, YOLO26 0.725, CLAD-Net
0.697이고, 팁 오차 중앙값은 **YOLO26이 3.45 px로 가장 작다**(YOLOv8s 3.60 px, CLAD-Net
4.93 px). 헝가리안 정밀도는 YOLOv8s 0.759, YOLO26 0.728, CLAD-Net 0.663이며, 속도는
YOLOv8s 4.89 ms, YOLO26 6.72 ms, CLAD-Net 8.78 ms로 파라미터가 가장 적은 CLAD-Net이
가장 느렸다. CLAD-Net과의 격차는 `tool` 클래스보다 **`tip` 클래스에서 두 배 이상** 벌어졌다.

**이 순위는 구현체와 초기 가중치를 바꿔도 유지된다.** 참조 구현으로 COCO 사전학습에서
학습한 두 회차를 넣어도 다섯 회차의 mAP@0.5 순서는 두 데이터셋 모두 YOLOv8s 계열,
YOLO26 계열, CLAD-Net 순이다. 사전학습과 참조 구현이 같은 아키텍처 안에서 만드는 개선폭은
mAP@0.5 +0.003〜0.010, 탐지 실패율 −0.3〜2.5 pp로, **아키텍처 사이의 격차(CLAD-Net과
YOLOv8s의 mAP@0.5 차이 0.054〜0.070)의 5분의 1 이하다.** 그 대신 사전학습은 **수렴을
절반으로 줄인다**: cholec80 최고 에포크가 62·64로, 스크래치의 107·108에서 앞당겨진다(§4.4).

학습 없이 수행한 라벨 할당 진단으로 그 원인을 좁혔다. CLAD-Net의 정적 앵커 비율 매칭은
`tool` GT 하나에 평균 16.2개, `tip` GT 하나에 9.0개의 양성 앵커를 배정해 **1.81 : 1의 신호
불균형**을 만드는 반면, 동적 할당을 쓰는 두 모델은 균형에 가깝다: YOLOv8s의
TaskAlignedAssigner [4] 가 9.6개와 9.8개(**0.98 : 1**), YOLO26이 9.6개와 9.9개
(**0.97 : 1**)다. 작은 객체가 구조적으로 불리해지는 정적 할당이 팁 클래스 격차의
직접적 원인으로 보인다.

**YOLO26의 end-to-end 헤드는 이 과제에서 뚜렷한 이점을 보이지 못했다.** NMS를 없앴는데도
오탐이 YOLOv8s보다 17 % 많고(헝가리안 10 px 기준 35,877 대 30,694), AP@0.5도 낮다. 대신
DFL을 버리고 거리를 직접 회귀한 덕에 **엄격한 IoU와 팁 좌표 정밀도에서는 앞선다**
(mAP@0.5:0.95 0.555 대 0.552, 팁 오차 중앙값 3.45 대 3.60 px). 이 성질도 참조 구현
회차에서 그대로 재현된다(0.564 대 0.558, 3.31 대 3.48 px).

**`tool` 상자 어노테이션이 팁 탐지에 기여하는 폭은 아키텍처에 달려 있다.** 재구현 세
회차를 `tip` 1클래스로 다시 학습해(여섯 셀) `tool` 감독 신호의 값을 쟀다. **CLAD-Net은 팁만
학습한 쪽이 오히려 낫고**(Hit@10 px +5.34·+5.60 pp, 탐지 실패율 −2.97·−1.48 pp), 앵커프리
두 모델은 조금 못하다(−0.16〜−2.33 pp). 학습 없이 라벨 할당을 다시 재어 원인을 좁혔다.
팁 GT가 받는 양성 앵커 수는 두 모드에서 **똑같고**(CLAD-Net 8.98개), 사라지는 것은 팁의
1.81배를 가져가던 `tool` 쪽 신호다. 정적 할당의 클래스 불균형이 처치로 없어지는 셈이다.
가장 나쁜 셀조차 손실이 2.33 pp로 **아키텍처 선택이 만드는 격차보다 작아**, 상자 레이블을
절반으로 줄이는 대가로는 싸다(§4.5).

**같은 아키텍처라면 참조 구현이 더 빠르지도 않았다.** 유휴 GPU에서 다섯 회차를 같은
구간으로 다시 재니, 네트워크와 후처리만으로는 참조 구현이 대등하거나 빠르지만
(3.94 대 4.26 ms, 6.22 대 6.20 ms) RGB 프레임 한 장을 상자로 바꾸는 실제 경로에서는
1.20〜1.28배 느리다. `ultralytics`의 예측 래퍼가 프레임당 1.85〜2.32 ms를 쓰기 때문이다(§3.5).

## 1. 서론 (Introduction)

### 1.1 연구 배경

최소침습수술에서 수술도구의 위치를 실시간으로 파악하는 것은 내시경 카메라 자동 유도,
술기 평가, 수술 단계 인식의 공통 전제다 [5]. 특히 도구의 **끝점**은 조직과 실제로 상호작용하는
지점이므로, 도구 전체의 위치보다 임상적 의미가 크다.

이 저장소의 주 프로젝트(tooltip-detector)는 이 문제를 **히트맵 회귀**로 푼다. 팁 위치를
가우시안 또는 거리 기반 맵으로 인코딩해 U-Net 계열 [6] 로 회귀하고, 피크를 추출해 좌표를
얻는다 [3]. 본 보고서는 같은 문제를 **객체 탐지**로 푸는 세 방법론을 검증한다.

### 1.2 비교 대상

**CLAD-Net** [1] 은 내시경 수술도구 탐지를 목표로 제안된 전용 아키텍처다. CSPDarknet53 백본에
교차층 집계 어텐션 넥(cross-layer aggregation attention)을 얹어, 복합 어텐션 모듈(CAM)과
정제 모듈(RM)로 다중 스케일 특징을 융합한다. 저자들은 Cholec80 부분집합에서 AP@0.5 98.9 %와
68.5 FPS를 보고하며, YOLOv5 [7]·YOLOX [8]·YOLOv6 [9]·RT-DETR [10] 등 10종 대비 우위를 주장한다.

**YOLOv8s** [2] 는 범용 단일 단계 탐지기다. CLAD-Net이 기반한 YOLOv5 계열과 비교해 네 가지가
바뀌었다. 앵커를 없앤 **앵커프리** 예측, 상자 변을 분포로 회귀하는 **DFL** [11], 정적 규칙 대신
예측 품질로 양성을 고르는 **TaskAlignedAssigner** [4], 그리고 C3를 대체한 **C2f** 블록이다.

**YOLO26** [14] 은 같은 계열의 최신 세대다. YOLOv8s와 비교해 다시 네 가지가 바뀌었다.
학습용 one2many 분기와 추론용 one2one 분기를 함께 두어 **NMS를 없앤 end-to-end 헤드**
[15], **DFL을 버리고**(`reg_max=1`) 네 거리를 직접 회귀하는 상자 표현, C2f를 대체한
**C3k2** 블록과 백본 말단의 **C2PSA** 어텐션 스테이지, 그리고 행렬 모양 파라미터에
직교화된 업데이트를 더하는 옵티마이저 **MuSGD**(Muon [16] + SGD)다.

**참조 구현 두 회차**는 같은 두 아키텍처를 `ultralytics`가 배포한 코드로, COCO 사전학습
가중치에서 파인튜닝한 것이다([`baseline/yolov8s`](../baseline/yolov8s/README.md) 8.4.128,
[`baseline/yolo26`](../baseline/yolo26/README.md) 8.4.131). 재구현 회차와 팁 상자 크기·프레임
샘플링·에포크·입력 크기를 맞췄으므로 **같은 아키텍처끼리 짝지어 읽을 수 있지만, 조건이 하나만
다른 것은 아니다**: 초기 가중치, 증강 레시피, 구현체가 함께 바뀐다(§2.4). 따라서 이 둘은
비교의 축이 아니라 **대조군**이며, "재구현이 아키텍처를 제대로 대변하는가"와 "사전학습은
얼마나 벌어 주는가"라는 두 질문에만 쓴다(§4.4).

> **혼동 주의.** [`baseline/yolo26clone`](../baseline/yolo26clone/README.md)(순수 PyTorch
> 재구현, 스크래치)과 [`baseline/yolo26`](../baseline/yolo26/README.md)(`ultralytics`,
> COCO 사전학습)은 **다른 회차다.** 본문에서 `YOLO26`은 앞의 것을, `YOLO26(참조)`는 뒤의
> 것을 가리킨다. `YOLOv8s`와 `YOLOv8s(참조)`도 같다.

### 1.3 기여

1. `ultralytics` 없이 세 아키텍처를 PyTorch로 재구현했다. YOLOv8s 재구현은 공개 체크포인트와
   **17개 모듈 전부에서 파라미터 수가 정확히 일치**하고(11,138,309개, `nc=7` 기준),
   YOLO26 재구현은 파라미터·출력·손실이 참조 구현과 **비트 단위로 일치**한다(§2.3).
2. 점 어노테이션을 고정 크기 상자로 바꾸는 정식화로, 탐지기와 세그멘테이션 기반 방법을
   **동일한 지표로** 비교할 수 있게 했다.
3. 세 탐지기의 성능 차이를 **학습 없이 측정 가능한 라벨 할당 통계**로 설명했다. 동적 할당을
   쓰는 두 세대(YOLOv8s·YOLO26)가 정적 할당(CLAD-Net)과 갈리는 지점을 직접 보였다.
4. 세 모델의 추론 경로에서 **정확도를 잃지 않는 최적화**를 찾아 적용했다(§3.5).
5. 같은 아키텍처의 **참조 구현·사전학습 회차를 대조군으로 붙여**, 아키텍처 순위가 구현체와
   초기 가중치에 좌우되지 않음을 확인하고 사전학습이 벌어 주는 폭을 수치로 묶었다(§4.4).
6. `tool` 상자 없이 팁만 학습한 여섯 회차를 더해, **부가 레이블이 팁 탐지에 기여하는 폭**을
   재고 그 값이 라벨 할당 방식에 따라 부호까지 갈린다는 것을 보였다(§4.5).

## 2. 방법 (Methods)

### 2.1 문제 정의: 점을 상자로

데이터셋은 도구마다 바운딩 상자와 팁 좌표를 갖고, **도구 종류 레이블은 없다.** 따라서 두
클래스를 정의했다.

표 1. 학습에 사용한 객체 클래스 정의

| 클래스 | 정의                                             |
| ------ | ------------------------------------------------ |
| `tool` | 어노테이션의 바운딩 상자 그대로                  |
| `tip`  | 팁 좌표를 중심으로 한 **32 × 32 px 정사각 상자** |

예측된 `tip` 상자의 중심이 팁 좌표다. 상자 한 변의 길이는 형식적 선택이 아니라 실제
하이퍼파라미터이며, 그 근거는
[CLAD-Net 보고서 부록 A](../baseline/cladnet/docs/experimental-results.md)에서 유도했다.
요약하면 이렇다. CLAD-Net의 objectness 학습 목표가 예측·GT 상자의 IoU이므로, 상자가 작으면 정확히
맞힌 예측조차 높은 신뢰도를 낼 수 없다. 초기 10 px 설정에서 팁 예측의 최대 신뢰도는 0.694였고,
32 px로 키운 뒤 0.925가 되었다.

### 2.2 아키텍처

세 모델 모두 백본에서 세 해상도의 특징(P3 / P4 / P5, stride 8 / 16 / 32)을 뽑아 넥에서 섞고,
그 셋을 각각 헤드에 넣어 예측한다는 큰 틀은 같다. 다른 것은 **넥이 어떻게 섞는가**와
**헤드가 무엇을 내놓는가**다.

#### CLAD-Net

![그림 1. CLAD-Net 구조. 텐서 모양은 본 재구현에서 실제로 뽑은 값이다.](../baseline/cladnet/docs/images/model-architecture.png)

벡터 원본은 [baseline/cladnet/docs/model-architecture.svg](../baseline/cladnet/docs/model-architecture.svg)에 있다.

논문의 기여는 전부 **넥**에 있다. 백본(CSPDarknet53)과 헤드는 YOLOv5 계열의 관습을 따르고,
넥에서 세 가지가 더해진다.

- **교차층 lateral 연결.** 백본의 C3 / C4 / C5를 depthwise separable conv로 통과시켜
  top-down 경로의 P3 / P4 / P5와 직접 잇는다. 얕은 층의 윤곽·경계 정보를 깊은 층에
  다시 넣어 상자 경계 회귀를 돕는다는 것이 논문의 주장이다.
- **RM(Refinement Module).** 그 lateral 특징과 top-down 특징 두 갈래에 각각 채널 가중치
  `ω₁`, `ω₂`를 구해 `ω = ω₁ + ω₂`로 합치고, `ω · F_a`와 `(1 − ω) · F_b`를 이어 붙인다.
  어느 쪽 정보를 얼마나 실을지 층마다 다시 정하는 장치다.
- **CAM(Composite Attention Mechanism).** bottom-up 경로에서 concat 직후에 놓인다.
  적응 풀링 네 스케일로 문맥을 모으는 AAB와, 채널별·픽셀별 두 갈래 채널 어텐션을 더하는
  MSAB를 병렬로 돌려 곱한 뒤 이어 붙이고 1 × 1로 채널을 줄인다.

헤드는 **앵커 기반 분리형**이다. 레벨마다 앵커 3개를 두고 셀마다 Obj / Reg / Cls를 예측하므로
640 × 640 입력에서 후보가 25,200개 나온다.

#### YOLOv8s

![그림 2. YOLOv8s 구조. 모듈 번호는 공개된 YOLOv8 구성을 따른다.](../baseline/yolov8sclone/docs/images/model-architecture.png)

벡터 원본은 [baseline/yolov8sclone/docs/model-architecture.svg](../baseline/yolov8sclone/docs/model-architecture.svg)에 있다.

넥은 평범한 PAN-FPN이고, 대신 **블록과 헤드**가 다르다.

- **C2f 블록.** YOLOv5의 C3를 대체한다. C3가 병목 사슬의 마지막 출력만 쓰는 반면 C2f는
  중간 출력을 전부 남겨 이어 붙이므로, 융합 conv가 `2 + n`갈래를 본다. 같은 채널 폭에서
  경사 경로가 늘어난다.
- **앵커프리 헤드.** 앵커 상자를 두지 않고, 셀 중심을 앵커 포인트로 삼아 상·하·좌·우
  네 방향 거리를 예측한다. 후보가 셀당 하나뿐이라 8,400개로 줄고, 데이터셋마다 앵커를
  다시 정할 필요가 없다.
- **DFL(Distribution Focal Loss) [11].** 각 변의 거리를 값 하나가 아니라 **16개 빈에 대한
  분포**로 예측하고, 그 기댓값을 거리로 쓴다. 경계가 흐리거나 가려진 상황에서 분포가
  퍼지는 방식으로 불확실성을 표현할 수 있다.
- **objectness가 없다.** 클래스 점수가 곧 신뢰도이며, 그 학습 목표는 아래 할당기가
  정한다.
- **TaskAlignedAssigner [4].** 정적 규칙 대신 `클래스 점수^0.5 × CIoU^6`로 정렬도를 재어
  GT마다 상위 10개를 양성으로 뽑는다. §4.1이 다루는 차이가 여기서 나온다.

#### YOLO26

![그림 3. YOLO26 구조. 모듈 번호는 공개된 YOLO26 구성을 따르며, 텐서 모양과 파라미터 수는 본 재구현에서 실제로 뽑은 값이다.](../baseline/yolo26clone/docs/images/model-architecture.png)

벡터 원본은 [baseline/yolo26clone/docs/model-architecture.svg](../baseline/yolo26clone/docs/model-architecture.svg)에 있다.

넥은 YOLOv8s와 같은 PAN-FPN이고, 바뀐 것은 **블록·헤드·상자 표현·옵티마이저** 넷이다.

- **end-to-end 헤드 [15].** 헤드가 두 벌이다. 학습용 `one2many` 분기는 GT마다 상위 10개를
  양성으로 받고, 추론용 `one2one` 분기는 상위 7개에 2차 top-k 1개를 더해 **객체당 상자
  하나만** 남도록 학습된다. 추론은 후자만 읽으므로 **NMS가 없다.** 손실은
  `w·L_one2many + (1 − w)·L_one2one`이고 `w`가 0.8에서 0.1로 선형 감쇠해, 초반에는 조밀한
  할당이 끌고 가다 후반에는 실제로 추론에 쓰이는 분기가 끌고 간다. `one2one` 분기는
  **detach된 특징**을 받아 공유 백본에 그래디언트를 보내지 않는다.
- **DFL이 없다.** `reg_max=1`이라 각 변을 분포가 아니라 **값 하나로** 회귀하고, 세 번째
  손실항이 DFL 대신 네 거리의 L1이다. 이산 빈의 해상도 한계가 없어지는 대신, 불확실성을
  분포로 표현하는 능력을 잃는다. §3.2의 팁 정밀도 차이가 여기서 나오는 것으로 보인다.
- **C3k2 + C2PSA.** C2f 자리에 C3k2가 들어가고, 백본 말단 SPPF 뒤에 **C2PSA** 어텐션
  스테이지가 붙는다.
- **라벨 할당에 두 가지가 더해진다.** TaskAlignedAssigner를 쓰되 (i) 한 변이 stride보다
  짧은 GT 상자를 stride 크기로 늘려 "상자 안에 들어오는 앵커"를 세고, (ii) 2차 top-k를
  적용한다. (i)은 작은 객체가 양성을 하나도 못 받는 것을 막는 장치인데, 32 px 팁 상자는
  letterbox 후 27.8 px이라 이 하한(16 px)에 걸리지 않는다.
- **MuSGD.** 모든 파라미터가 SGD 업데이트를 받고, 행렬 모양 파라미터는 Newton-Schulz
  반복으로 직교화한 업데이트를 하나 더 받는다(Muon [16]). 분류 헤드는 학습률 3배다.
  **세 모델 중 이 모델만 옵티마이저가 다르다**(§2.4).

표 2. 세 모델의 주요 구조 차이

|              | CLAD-Net                                  | YOLOv8s                             | YOLO26                                             |
| ------------ | ----------------------------------------- | ----------------------------------- | -------------------------------------------------- |
| 백본         | CSPDarknet53 (C3 블록)                    | CSPDarknet (C2f 블록)               | C3k2 + SPPF + **C2PSA** 어텐션                     |
| 넥           | 교차층 집계 어텐션 (CAM + RM)             | PAN-FPN                             | PAN-FPN                                            |
| 헤드         | 분기 하나                                 | 분기 하나                           | **분기 둘** (one2many + one2one) [15]              |
| 추론 후처리  | NMS                                       | NMS                                 | **없음** (객체당 상자 하나, top-k만)               |
| 상자 표현    | 앵커 상대 offset                          | 앵커프리(4방향 거리)                | 앵커프리(4방향 거리)                               |
| 상자 회귀    | 값 1개                                    | **DFL**(변마다 16개 빈) [11]        | **DFL 없음** (`reg_max=1`), 세 번째 항이 L1        |
| Objectness   | 있음                                      | 없음                                | 없음                                               |
| 라벨 할당    | 앵커 **모양** 비율 매칭(≤ 4배) + 이웃 셀  | **TaskAlignedAssigner** [4]         | 같음 + **상자 크기 하한** + **2차 top-k**          |
| 손실         | `0.05·CIoU + 1.0·BCE(obj) + 0.5·BCE(cls)` | `7.5·CIoU + 0.5·BCE(cls) + 1.5·DFL` | `7.5·CIoU + 0.5·BCE(cls) + 1.5·L1`, 두 분기 가중합 |
| 옵티마이저   | SGD                                       | SGD                                 | **MuSGD** (Muon [16] + SGD)                        |
| 후보 상자 수 | 25,200                                    | 8,400                               | 8,400 (분기당)                                     |
| 파라미터     | **7,488,483**                             | 11,136,374                          | 9,949,412                                          |

CIoU [12] 는 세 모델 공통이다.

### 2.3 재구현 검증

CLAD-Net은 저자 공개 코드가 없어 논문 Fig. 1〜3만으로 재구성했고, 본문과 도해가 어긋나는
지점이 여덟 곳 있었다(예: 본문은 bottom-up 경로를 "upsampling"이라 쓰지만 그 방향은 해상도가
줄어드는 쪽이다).  **구조 해석이 맞는지에 대한
독립 검증은 파라미터 수 일치(7.488 M 대 논문 보고 7.5 M)뿐이다.**

YOLOv8s 재구현은 공개된 사전학습 체크포인트 `yolov8s_cholec80.pt`(Cholec80 도구 7종으로
학습된 것, [baseline/yolov8s](../baseline/yolov8s/README.md))의 state_dict와 모듈 단위로 대조해
17개 모듈 전부에서 파라미터 수가 일치함을 확인했다. **가중치는 가져오지 않았고 구조 대조에만
썼다.** 그 체크포인트는 클래스 구성이 다르고, 언피클하려면 `ultralytics`가 필요하다.

YOLO26 재구현은 **세 모델 중 가장 강하게 검증됐다.** 참조 체크포인트를 평문 텐서로 추출해
네 단계를 대조했고, 네 단계 모두 통과한다.

표 3. YOLO26 재구현 검증 (`run verify-clone`)

| 질문                     | 확인 방법                           | 결과                           |
| ------------------------ | ----------------------------------- | ------------------------------ |
| 모양이 맞는가            | 모듈별 파라미터 수                  | 10,009,784 (`nc=80`) 완전 일치 |
| 파라미터가 맞는가        | 참조 `state_dict`를 그대로 로드     | 708 / 708 키, 누락 0, 초과 0   |
| **같은 것을 계산하는가** | 참조의 입력으로 참조의 출력을 재현  | **최대 오차 0.000e+00**        |
| **같은 것을 학습하는가** | 참조의 GT 상자로 참조의 손실을 재현 | **최대 오차 0.000e+00**        |

마지막 두 항목이 중요하다. assigner와 손실 함수는 대조할 가중치가 없으므로 파라미터 수만
맞춰서는 검증되지 않는 부분이며, 실제로 이 대조가 잡아낸 버그가 있었다. 참조 구현은 모델을
만든 뒤 **모든 BatchNorm의 `eps`를 1e-3, `momentum`을 0.03으로 덮어쓴다**(PyTorch 기본값은
1e-5와 0.1). 파라미터 수도 `state_dict` 키도 이 값에 영향받지 않으므로 앞의 두 단계는
통과하지만, 출력은 첫 Conv 레이어에서부터 어긋났다.

**다만 이 검증은 구조·손실이 참조와 같음을 보인 것이고, 학습 결과가 참조와 같음을 보인 것은
아니다.** 후자는 [YOLO26 보고서 §6](../baseline/yolo26clone/docs/experimental-results.md)에서
따로 다룬다(사전학습 없이도 mAP@0.5 격차가 0.003〜0.010에 머문다).

### 2.4 학습 및 평가 프로토콜

표 4. 재구현 세 회차에 공통으로 적용한 학습 및 평가 프로토콜

| 항목               | 값                                                                                            |
| ------------------ | --------------------------------------------------------------------------------------------- |
| 입력               | 640 × 640 letterbox (원본 736 × 480)                                                          |
| 에포크 / 배치 / lr | 150 / 16 / 0.01                                                                               |
| 옵티마이저         | SGD (momentum 0.937, nesterov). **YOLO26만 MuSGD, momentum 0.9**                              |
| 증강               | mosaic + 좌우 반전 + HSV 지터                                                                 |
| 공통 추가          | warmup 3 에포크, cosine 감쇠, 가중치 EMA, 그래디언트 클리핑 10                                |
| `--frame-stride`   | 5 (학습 프레임 5장 중 1장)                                                                    |
| 사전학습           | 없음 (무작위 초기화)                                                                          |
| 평가               | `conf=0.25`, NMS IoU 0.45 (**YOLO26은 NMS가 없어 `max_det=300`**). AP 곡선은 `conf=0.001`까지 |

**팁 상자 크기, 프레임 샘플링, 에포크, 입력 크기, 증강이 세 모델에서 모두 같다.** 다른 것은
두 가지뿐이다: YOLO26의 옵티마이저(MuSGD)와, NMS 대신 `max_det`이 후보 수를 제한한다는 점이다.
둘 다 YOLO26 설정의 일부이므로, **이 보고서의 비교는 "아키텍처만의 비교"가 아니라 "설정
전체의 비교"로 읽어야 한다.** 옵티마이저 몫을 떼어 내려면 `--optimizer sgd` 실행을 따로
쌓아야 하고, 이번 실험에서는 하지 않았다.

#### 참조 구현 두 회차는 무엇이 다른가

표 4-1. 대조군(참조 구현)과 재구현의 조건 차이

| 항목                             | 재구현 세 회차                 | 참조 구현 두 회차                                                                           |
| -------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------- |
| 구현체                           | 본 저장소의 PyTorch 코드       | `ultralytics` 8.4.128 / 8.4.131                                                             |
| 초기 가중치                      | 무작위                         | **COCO 사전학습** (`yolov8s.pt`, `yolo26s.pt`)                                              |
| 증강                             | mosaic + 좌우 반전 + HSV       | 여기에 **이동 0.1 · 스케일 0.5 아핀, erasing 0.4, 마지막 10 에포크 mosaic 해제**가 더해진다 |
| 옵티마이저                       | SGD (YOLO26만 MuSGD)           | YOLOv8s(참조) SGD, YOLO26(참조) `auto` → MuSGD                                              |
| val 표본                         | 1,500 프레임                   | 1,903〜1,914 프레임, 각 프로젝트의 검증 코드로 채점                                         |
| 팁 상자 / stride / 에포크 / 입력 | 32 px / 5 / 150 / 640 × 640    | 같음                                                                                        |
| test 채점                        | 같은 test 전수, 같은 매칭 규칙 | 같음                                                                                        |

**test 채점만 완전히 같고 학습 조건은 세 가지가 함께 다르다.** 그래서 §4.4의 격차는
"사전학습의 효과"가 아니라 **"사전학습 + 증강 레시피 + 구현체를 한꺼번에 바꿨을 때의 효과"**로
읽어야 한다. 셋을 나누려면 `--no-pretrained` 회차가 필요하고, 명령은 두 서브 프로젝트의
`docs/commands.md`에 준비돼 있지만 아직 돌리지 않았다.

val 곡선(그림 8)의 **높이**는 계열을 넘어 비교하지 않는다. 표본도 채점 코드도 다르기
때문이며, 모델 간 비교는 전부 test 수치로 한다.

### 2.5 데이터셋

표 5. 데이터셋별 분할과 평가 대상 규모

| 데이터셋        | train (stride 5) |    val |   test | test GT 팁 |               세션 |
| --------------- | ---------------: | -----: | -----: | ---------: | -----------------: |
| `cholec80` [13] |  71,032 → 14,207 | 15,312 | 98,234 |    161,969 | 40편 (video41〜80) |
| `erop`          | 108,424 → 21,685 | 36,140 | 36,142 |     50,718 |              5세션 |

`cholec80`은 **영상 단위로 분리된** 분할이다(video01〜32 train / 33〜40 val / 41〜80 test).
`erop`은 영상 내부를 프레임 단위로 섞어 나눈 분할이라 인접 프레임이 train과 test에 함께
들어간다(시간적 누수). **따라서 `erop` 수치는 낙관적으로 읽어야 하며, 본 보고서의 결론은
`cholec80`을 기준으로 삼는다.**

## 3. 결과 (Results)

### 3.1 탐지 성능

![그림 4. 클래스별 탐지 성능. 격차는 tip 클래스에서 벌어진다. 옅은 막대 둘은 참조 구현(COCO 사전학습) 회차이며, 바로 옆의 진한 막대가 같은 아키텍처의 스크래치 재구현이다. 세그멘테이션 기반 방법은 상자를 내놓지 않으므로 AP를 정의할 수 없다.](figures/baseline-report-detection-ap.png)

표 6. 데이터셋과 모델별 객체 탐지 성능 (재구현 셋이 비교의 축, 참조 구현 둘은 대조군)

| 데이터셋   | 모델          |   mAP@0.5 | mAP@0.5:0.95 | tool AP@0.5 | tip AP@0.5 | tip AP@0.5:0.95 |
| ---------- | ------------- | --------: | -----------: | ----------: | ---------: | --------------: |
| `cholec80` | YOLOv8s(참조) |   *0.755* |      *0.558* |     *0.838* |    *0.671* |         *0.413* |
| `cholec80` | **YOLOv8s**   | **0.751** |        0.552 |   **0.835** |  **0.666** |           0.408 |
| `cholec80` | YOLO26(참조)  |   *0.735* |      *0.564* |     *0.827* |    *0.643* |         *0.425* |
| `cholec80` | YOLO26        |     0.725 |    **0.555** |       0.817 |      0.632 |       **0.415** |
| `cholec80` | CLAD-Net      |     0.697 |        0.472 |       0.817 |      0.577 |           0.303 |
| `erop`     | YOLOv8s(참조) |   *0.865* |      *0.695* |     *0.901* |    *0.828* |         *0.579* |
| `erop`     | **YOLOv8s**   | **0.858** |        0.685 |   **0.895** |  **0.821** |           0.567 |
| `erop`     | YOLO26(참조)  |   *0.848* |      *0.701* |     *0.892* |    *0.803* |         *0.590* |
| `erop`     | YOLO26        |     0.844 |    **0.692** |       0.889 |      0.799 |       **0.579** |
| `erop`     | CLAD-Net      |     0.788 |        0.575 |       0.878 |      0.698 |           0.410 |

굵은 값은 **재구현 세 회차 안에서의** 최고값이고, 기울인 값은 대조군인 참조 구현 회차다.
조건이 다르므로(§2.4) 굵기 경쟁에 넣지 않는다.

**두 범용 탐지기가 전용 아키텍처를 앞선다.** 그리고 그 격차는 `tip` 클래스에 몰려 있다.
cholec80에서 YOLOv8s와 CLAD-Net의 `tool` AP@0.5 차이는 +0.018인데 `tip`은 +0.089로 다섯
배다. erop에서는 +0.017 대 +0.123으로 더 벌어진다. 이 비대칭이 §4 분석의 출발점이다.

**YOLO26은 느슨한 IoU에서 지고 엄격한 IoU에서 이긴다.** mAP@0.5는 YOLOv8s보다 낮은데
(−0.026, −0.014) mAP@0.5:0.95는 높고(+0.003, +0.007), `tip` 클래스에서 그 대비가 가장
뚜렷하다(cholec80 AP@0.5 −0.034 대 AP@0.5:0.95 +0.007). **상자를 덜 찾는 대신 찾은 상자는
더 정확하다**는 뜻이며, DFL 없이 거리를 직접 회귀하는 표현(§2.2)의 성질로 읽힌다.
AP@0.5가 낮은 쪽은 후보를 덜 낸 결과다: AP 곡선용 `conf=0.001`에서 `tip` 재현율이 0.893으로
YOLOv8s의 0.910에 못 미친다.

**대조군이 두 관찰을 모두 되풀이한다.** 참조 구현 회차 사이에서도 YOLOv8s(참조)가
YOLO26(참조)를 앞서고(mAP@0.5 0.755 대 0.735) 둘 다 CLAD-Net(0.697, 재구현뿐이다)보다
크게 높다. YOLO26 계열이 mAP@0.5에서 지고 mAP@0.5:0.95에서 이기는 성질(0.564 대 0.558)도
그대로다. **아키텍처 순위는 구현체나 초기
가중치가 아니라 아키텍처에서 온다**는 뜻이다. 사전학습이 벌어 주는 폭 자체는 §4.4에서 따로 센다.

### 3.2 팁 위치 정확도

![그림 5. 거리 임계값 전 구간의 누적 적중률. 검출기는 근거리에서, 세그멘테이션은 원거리에서 앞선다. 세로축이 오른쪽 끝에서 도달하는 높이가 100 %에서 탐지 실패율을 뺀 값이다.](figures/baseline-report-hit-curve.png)

표 7. 모델별 팁 위치 정확도와 탐지 실패율

| 데이터셋   | 모델                            | 탐지 실패율 |   Hit@10 px |   Hit@20 px |   Hit@50 px |      중앙값 |           P90 |
| ---------- | ------------------------------- | ----------: | ----------: | ----------: | ----------: | ----------: | ------------: |
| `cholec80` | YOLOv8s(참조)                   |    *6.34 %* |   *62.90 %* |   *69.01 %* |   *75.80 %* |   *3.48 px* |   *160.06 px* |
| `cholec80` | **YOLOv8s**                     |      8.82 % | **59.87 %** |     65.48 % |     71.78 % |     3.60 px |     181.03 px |
| `cholec80` | YOLO26(참조)                    |    *7.45 %* |   *61.57 %* |   *67.02 %* |   *73.31 %* |   *3.31 px* |   *177.14 px* |
| `cholec80` | YOLO26                          |      9.71 % |     59.27 % |     64.51 % |     70.44 % | **3.45 px** |     186.88 px |
| `cholec80` | CLAD-Net                        |      9.56 % |     56.65 % |     63.12 % |     69.96 % |     4.93 px |     184.62 px |
| `cholec80` | tooltip-detector `gaussian-tip` |      7.48 % |     59.55 % | **67.45 %** |     74.72 % |     5.39 px |     167.34 px |
| `cholec80` | tooltip-detector `gradient-seg` |  **2.90 %** |     30.04 % |     55.58 % | **76.12 %** |    16.12 px | **156.66 px** |
| `erop`     | YOLOv8s(참조)                   |    *4.31 %* |   *76.85 %* |   *80.22 %* |   *84.00 %* |   *1.97 px* |    *83.52 px* |
| `erop`     | **YOLOv8s**                     |  **4.79 %** | **75.91 %** |     79.28 % |     83.15 % |     2.04 px |      92.05 px |
| `erop`     | YOLO26(참조)                    |    *5.65 %* |   *74.59 %* |   *77.50 %* |   *81.06 %* |   *1.90 px* |   *116.87 px* |
| `erop`     | YOLO26                          |      5.96 % |     74.24 % |     77.37 % |     80.93 % | **1.96 px** |     112.90 px |
| `erop`     | CLAD-Net                        |      6.84 % |     68.47 % |     72.77 % |     77.03 % |     3.23 px |     159.00 px |
| `erop`     | tooltip-detector `gaussian-tip` |      5.03 % |     69.52 % | **79.36 %** | **83.82 %** |     7.81 px |  **74.28 px** |
| `erop`     | tooltip-detector `gradient-seg` |      3.09 % |     44.98 % |     70.17 % |     82.95 % |    10.77 px |      88.68 px |

**잡은 팁을 가장 정확히 짚는 것은 YOLO26이다.** 오차 중앙값이 cholec80 3.45 px, erop
1.96 px로 재구현 세 회차와 세그멘테이션 두 회차를 통틀어 가장 작다. 반대로 **놓치는 팁은 가장 많은 축이다**(탐지 실패율
9.71 %, 5.96 %). 두 성질이 함께 나타나는 것은 §3.1에서 본 "덜 찾고 정확히 찾는다"와 같은
현상이다. 꼬리에서도 불리하다: P90 오차가 erop에서 112.90 px로 YOLOv8s(92.05 px)보다 크다.

**참조 구현 회차에서도 두 성질이 같이 나타난다.** YOLO26(참조)의 오차 중앙값은 두 데이터셋
모두 표 7 전체에서 가장 작고(3.31 px, 1.90 px), 탐지 실패율은 YOLOv8s(참조)보다 높다
(7.45 % 대 6.34 %, 5.65 % 대 4.31 %). 다만 **사전학습이 실패율을 눈에 띄게 낮춘다**:
cholec80에서 두 아키텍처 모두 2.3〜2.5 pp 내려간다(§4.4).

![그림 6. 팁 오차 거리 분포(매칭된 팁 기준, 로그 축). 일곱 모델 모두 이봉형이다.](figures/baseline-report-error-distribution.png)

### 3.3 운용 지점에서의 헝가리안 1:1 매칭

적중률은 오탐을 세지 않는다. 예측과 GT를 1:1로 강제 매칭하면 정밀도가 드러난다.

![그림 7. 헝가리안 1:1 매칭의 정밀도와 재현율. 점은 거리 상한 10 / 20 / 50 px이며, 오른쪽 위로 갈수록 좋다. 거리 상한은 가장 한산한 CLAD-Net 곡선에만 표시했고 나머지 곡선도 같은 순서로 지난다.](figures/baseline-report-precision-recall.png)

표 8. 거리 상한 10 px에서의 헝가리안 1:1 매칭 성능

| 데이터셋   | 모델           | cap   |        TP |       FP | Precision |    Recall |
| ---------- | -------------- | ----- | --------: | -------: | --------: | --------: |
| `cholec80` | YOLOv8s(참조)  | 10 px | *101,826* | *36,601* |   *0.736* |   *0.629* |
| `cholec80` | **YOLOv8s**    | 10 px |    96,914 |   30,694 |     0.759 | **0.598** |
| `cholec80` | YOLO26(참조)   | 10 px |  *99,687* | *40,315* |   *0.712* |   *0.616* |
| `cholec80` | YOLO26         | 10 px |    95,944 |   35,877 |     0.728 |     0.592 |
| `cholec80` | CLAD-Net       | 10 px |    91,698 |   46,641 |     0.663 |     0.566 |
| `cholec80` | `gaussian-tip` | 10 px |    96,297 |   28,526 | **0.772** |     0.595 |
| `erop`     | YOLOv8s(참조)  | 10 px |  *38,962* |  *7,892* |   *0.832* |   *0.768* |
| `erop`     | **YOLOv8s**    | 10 px |    38,484 |    7,914 | **0.829** | **0.759** |
| `erop`     | YOLO26(참조)   | 10 px |  *37,816* |  *7,994* |   *0.826* |   *0.746* |
| `erop`     | YOLO26         | 10 px |    37,641 |    8,360 |     0.818 |     0.742 |
| `erop`     | CLAD-Net       | 10 px |    34,719 |   13,924 |     0.714 |     0.685 |
| `erop`     | `gaussian-tip` | 10 px |    35,240 |    8,199 |     0.811 |     0.695 |

**CLAD-Net의 오탐이 가장 많다.** cholec80에서 FP 46,641개로 YOLOv8s(30,694)의 1.5배,
`gaussian-tip`(28,526)의 1.6배다.

**사전학습은 재현율을 사고 정밀도를 판다.** cholec80에서 YOLOv8s(참조)는 TP를 4,912개
늘리지만 FP도 5,907개 늘려, 재현율은 0.598에서 0.629로 오르는 대신 정밀도가 0.759에서
0.736으로 떨어진다. YOLO26 계열도 같은 방향이다(재현율 +0.024, 정밀도 −0.016).
운용 지점에서 무엇을 얻는지는 응용이 정한다.

**그리고 NMS를 없앤 것이 오탐을 줄이지 못했다.** YOLO26의 FP는 35,877개로 YOLOv8s보다
17 % 많다. end-to-end 헤드의 설계 의도는 "객체당 상자 하나"이므로 중복 상자가 줄어들 것을
기대할 수 있지만, 이 데이터에서는 그렇게 나타나지 않았다. 중복 상자를 지우는 문제가 아니라
**어느 객체에 상자를 낼지 고르는 문제**가 남아 있다는 뜻이며, §6.1의 과소 탐지 분석과
같은 방향을 가리킨다.

### 3.4 학습 수렴

![그림 8. 에포크별 val mAP@0.5:0.95. 점은 최고 에포크다. 스크래치 두 범용 탐지기의 곡선은 거의 겹치고, 사전학습에서 시작한 참조 구현 둘은 훨씬 일찍 정점을 지나며, CLAD-Net만 따로 간다. 계열마다 val 표본과 채점 코드가 다르므로 곡선의 높이는 계열 안에서만 견준다.](figures/baseline-report-convergence.png)

표 9. 모델별 초기 수렴, 최고 성능 도달 시점, 학습 시간

| 데이터셋   | 모델          | 2 에포크 mAP@0.5 |   최고 에포크 | 최고 mAP@0.5:0.95 | 학습 시간 |
| ---------- | ------------- | ---------------: | ------------: | ----------------: | --------: |
| `cholec80` | YOLOv8s(참조) |          *0.611* |    *62* / 150 |           *0.603* |   *3.4 h* |
| `cholec80` | YOLOv8s       |        **0.456** |     108 / 150 |             0.534 | **4.1 h** |
| `cholec80` | YOLO26(참조)  |          *0.656* |    *64* / 150 |           *0.602* |   *5.0 h* |
| `cholec80` | YOLO26        |            0.399 |     107 / 150 |         **0.543** |     5.8 h |
| `cholec80` | CLAD-Net      |            0.125 | **145** / 150 |             0.455 |     9.2 h |
| `erop`     | YOLOv8s(참조) |          *0.733* |   *110* / 150 |           *0.702* |   *7.5 h* |
| `erop`     | YOLOv8s       |        **0.699** |     125 / 150 |             0.719 | **5.8 h** |
| `erop`     | YOLO26(참조)  |          *0.711* |   *103* / 150 |           *0.703* |   *7.4 h* |
| `erop`     | YOLO26        |            0.589 |     123 / 150 |         **0.723** |     8.2 h |
| `erop`     | CLAD-Net      |            0.310 | **147** / 150 |             0.620 |    14.2 h |

`최고 mAP@0.5:0.95`는 각 프로젝트가 자기 val 표본을 자기 코드로 채점한 값이므로 **계열을
넘어 비교하지 않는다**(§2.4). 계열 안에서 비교할 수 있는 것은 **에포크 축**이다.

세 가지가 읽힌다. 먼저 **초반 상승 속도.** 스크래치로 학습한 두 범용 탐지기는 2 에포크에
mAP@0.5 0.40〜0.46에 도달하는데 CLAD-Net은 0.125다. 다음은 **종착점.** YOLOv8s과 YOLO26은
107〜125 에포크에서 정점을 찍고 과적합으로 돌아서지만, CLAD-Net은 150 에포크에서도
최고점이 끝에 있어 아직 포화하지 않았다.

세 번째가 새로운 관찰이다. **YOLOv8s과 YOLO26의 최고점이 거의 같은 에포크에 있다**
(108 대 107, 125 대 123). 그림 8에서 두 곡선이 거의 겹친다. 데이터를 소진하는 시점이
모델이 아니라 **학습 프레임 수**(14,207·21,685장)로 정해진다는 뜻으로 읽힌다. 다만 종착
높이는 YOLO26이 조금 높다(val mAP@0.5:0.95 0.543 대 0.534, 0.723 대 0.719). §3.1에서 본
"엄격한 IoU에서 앞선다"가 학습 곡선에도 그대로 나타난다.

**사전학습은 그 시점을 절반으로 당긴다.** cholec80에서 참조 구현 둘은 62·64 에포크에
정점을 찍는다. 스크래치 회차의 107·108의 **약 60 %**다. 2 에포크 성적도 0.611·0.656으로
스크래치의 0.456·0.399를 크게 앞선다. 반면 `erop`에서는 이 이점이 거의 사라져 최고
에포크가 110·103으로 스크래치(125·123)와 비슷하다. 시간적 누수가 있는 분할이라
스크래치 학습도 일찍 포화하기 때문으로 보인다. 어느 쪽이든 **150 에포크는 다섯 회차 모두에게
과하고, CLAD-Net에게만 모자란다.**

### 3.5 속도

다섯 회차를 **유휴 상태의 같은 GPU(RTX A6000 `cuda:2`)에서, 같은 프레임으로, 같은 두 구간**에
대해 다시 쟀다. `summary.json`의 `ms_per_frame`은 평가를 돌린 GPU가 제각각이고 계열마다
측정 구간도 달라(참조 구현은 전처리까지, 재구현은 letterbox 뒤부터) 그대로 쓸 수 없다.

> **측정 조건.** cholec80 test에서 고르게 뽑은 실제 프레임 60장, 배치 1, 워밍업 10장 뒤
> 5회 반복 평균. 모든 모델이 Conv+BN을 접은(fuse) 상태이고, `ultralytics`는 `AutoBackend`가
> 로드할 때 이것을 자동으로 한다. 두 구간은 이렇다.
> **(A) 네트워크**는 장치에 올라간 텐서에서 시작해 forward·디코딩·후처리까지,
> **(B) 프레임**은 RGB 넘파이 배열 한 장에서 시작해 letterbox와 장치 전송까지 포함한 전 경로다.
> 운용에서 실제로 치르는 값은 (B)다.

![그림 9. 속도 대 정확도. 가로축은 (B) 구간이다. 왼쪽 위로 갈수록 좋다. 표식 크기는 파라미터 수에 비례한다.](figures/baseline-report-speed-accuracy.png)

표 10. 모델별 추론 속도, 파라미터 수, 후보 상자 수 (RTX A6000 `cuda:2`)

| 모델                     |   파라미터 | (A) 네트워크 |  (B) 프레임 |     FPS |            후보 상자 수 |
| ------------------------ | ---------: | -----------: | ----------: | ------: | ----------------------: |
| **YOLOv8s**              |    11.14 M |      4.26 ms | **4.89 ms** | **205** |                   8,400 |
| YOLOv8s(참조)            |    11.14 M |    *3.94 ms* |   *6.26 ms* |   *160* |                   8,400 |
| YOLO26                   |     9.95 M |      6.20 ms |     6.72 ms |     149 |          8,400 (분기당) |
| YOLO26(참조)             |     9.95 M |    *6.22 ms* |   *8.07 ms* |   *124* |          8,400 (분기당) |
| CLAD-Net                 | **7.49 M** |      8.14 ms |     8.78 ms |     114 |                  25,200 |
| tooltip-detector `monai` |    11.45 M |      11.9 ms |      미측정 |      84 | 해당 없음 (히트맵 피크) |

재구현 셋의 (A) 열은 형제 보고서가 따로 보고한 값(4.62 · 5.96 · 8.44 ms)과 5〜8 % 안에서
일치하고 순위도 같다. 그 보고서의 측정은 프레임을 미리 letterbox해 장치에 올려 둔 것이라
여기의 (A)와 같은 구간이며, 차이는 실행 시점의 잡음이다. 루트 프로젝트 `monai`의 11.9 ms는
그 프로젝트가 보고한 값을 옮긴 것이고 fuse는 아직 적용하지 않았다.

다섯 회차 모두 **Conv+BN을 합성곱에 접은(fuse) 상태**가 운용값이다. eval 모드의 BatchNorm은
고정 아핀 변환이므로 앞의 합성곱에 접을 수 있고, 탐지 지표는 test 전수에서 헝가리안 TP가
0.01〜0.02 % 움직이는 데 그친다. **순위는 접기 전후로 바뀌지 않는다.**

**같은 아키텍처라면 참조 구현이 더 빠르지 않다.** (A) 구간에서 YOLOv8s(참조)는 3.94 ms로
재구현(4.26 ms)보다 빠르고 YOLO26 둘은 사실상 같지만(6.22 대 6.20 ms), 운용값인 (B)에서는
참조 구현이 1.20〜1.28배 **느리다**. 차이는 네트워크가 아니라 그 앞뒤다. `ultralytics`의
`predict` 래퍼가 프레임당 **1.85〜2.32 ms**를 쓰는 데 반해, 재구현의 letterbox와 장치 전송은
**0.52〜0.64 ms**로 끝난다. **아키텍처를 고르는 문제와 실행 경로를 고르는 문제가 서로
독립이라는 사례다.**

**파라미터가 가장 적은 CLAD-Net이 가장 느리다.** 논문이 주장하는 "더 적은 파라미터로 더
빠르다"는 성질은 이 구현·이 과제에서 재현되지 않았다. 이유는 둘로 보인다. 후보가 25,200개로
YOLOv8s의 3배여서 NMS 비용이 크고, CAM이 레벨마다 적응 풀링·업샘플을 4회씩 수행한다
(P3에서 80 × 80 × 336 텐서 대상). 파라미터로 드러나지 않는 연산이며, **BatchNorm이 없어
fuse의 혜택도 받지 못한다.**

**NMS를 없앤 YOLO26이 NMS를 도는 YOLOv8s보다 37 % 느리다.** 파라미터가 11 % 적은데도
그렇다. 두 가지가 겹친다. 첫째, `Detect.forward`가 추론에서도 `one2many` 분기를 계산하는데
읽지는 않았다. 이 낭비가 프레임당 약 1.2 ms였고 건너뛰도록 고쳤다(표의 값은 모두 고친
뒤의 것이다). 둘째, 그렇게 고친 뒤에도 백본이 더 무겁다. 형제 보고서가 접기 전 상태에서
구간을 쪼개 보니 YOLO26의 백본 + 넥만 6.09 ms로 YOLOv8s의 전체 forward보다 오래 걸렸고,
눈에 띄는 후보였던 C2PSA 어텐션은 0.60 ms에 불과했다
([YOLO26 클론 보고서](../baseline/yolo26clone/docs/experimental-results.md)). 원인은 한
곳이 아니라 **C3k2 블록 구성 전반**에 퍼져 있다.
**파라미터 수가 실행 시간을 예측하지 못하는 전형적인 사례다.** 참조 구현에서도 같다:
같은 아키텍처를 같은 장치에서 돌려도 (B) 구간이 1.20〜1.28배 벌어진다.

### 3.6 정성 결과와 데모 도구

다섯 서브 프로젝트 모두 학습한 모델을 영상에 바로 돌려 보는 GUI 데모를 갖고 있다(`run demo`).
원본 영상이나 추출 프레임을 골라 재생하면서 신뢰도·NMS 임계값을 바꾸고, 어노테이션을
겹쳐 볼 수 있다. **YOLO26 데모에는 IoU 슬라이더가 없다**: NMS를 돌리지 않으므로 있어도
아무 일도 하지 않는 조작이 되기 때문이다.

![그림 10. YOLOv8s 데모 실행 화면. cholec80 test의 video41 첫 프레임에서 도구 상자 두 개(초록)와 팁 상자 두 개(빨강, 중심에 십자)를 찾았고, 흰색 상자와 원이 어노테이션이다.](figures/baseline-report-demo.png)

화면이 보고서의 수치를 그대로 보여 준다. 상단 상태줄의 `cholec80 ep108 tip32 val mAP@0.5 0.737`은
§3.4에서 확인한 최고 에포크(108)와 §2.1의 팁 상자 크기(32 px)를 그대로 읽은 것이고,
하단 정보창의 `inference: 5.6 ms (179.1 FPS)`는 화면을 캡처한 시점의 값으로, Conv+BN을
접기 전에 측정된 것이다. §3.5의 값과 범위가 다른 것은 그 때문이며, §3.5는 test 전수 평균이라
프레임 읽기 비용도 함께 들어가 있다.

`tool 0.91`·`tool 0.93`과 `tip 0.76`·`tip 0.80`이라는 신뢰도는 §4.2에서 다루는
"32 px 팁 상자에서는 신뢰도 천장이 사라졌다"는 관찰의 한 사례다. 팁 상자 중심의 십자가
흰색 어노테이션 원 안에 들어와 있는 것도 확인할 수 있다.

---

## 4. 분석: 성능 차이는 어디에서 오는가

§3.1이 보여준 비대칭(`tool` 격차는 작고 `tip` 격차는 크다)을 설명하려면, 세 모델이 **무엇을
양성 예측으로 삼아 학습하는지**를 봐야 한다.

### 4.1 라벨 할당 밀도: 직접 측정

학습 없이, `cholec80` val 400 프레임에 대해 세 모델의 할당기를 한 번씩 돌려 "GT 상자 하나가
몇 개의 양성 앵커를 받는가"를 셌다. 참조 구현 두 회차는 아키텍처도 할당기도 재구현과 같고
할당 규칙은 가중치와 무관하므로 따로 재지 않았다. YOLO26은 두 분기 중 **공유 백본을 실제로 학습시키는
`one2many` 분기**를 측정했다(topk도 10이라 YOLOv8s과 직접 비교된다).

![그림 11. GT 상자 하나가 받는 양성 앵커 수. CLAD-Net만 tool에 tip의 약 두 배를 배정한다. (a) 클래스별 학습 신호량, (b) 양성 앵커가 놓이는 특징 레벨. cholec80 val 400 프레임.](figures/baseline-report-assignment.png)

표 11. GT당 양성 할당 수와 클래스 간 불균형

| 모델     |   `tool` 양성/GT |   `tip` 양성/GT | **tool : tip** |
| -------- | ---------------: | --------------: | -------------: |
| CLAD-Net | 16.2 (중앙값 18) |  9.0 (중앙값 9) |       **1.81** |
| YOLOv8s  |  9.6 (중앙값 10) | 9.8 (중앙값 10) |       **0.98** |
| YOLO26   |  9.6 (중앙값 10) | 9.9 (중앙값 10) |       **0.97** |

**CLAD-Net만 도구에 팁의 약 두 배를 배정한다.** 동적 할당을 쓰는 두 모델은 사실상 균형이며,
서로 구분되지 않는다(0.98 대 0.97). 이유는 할당 규칙에 있다.

- CLAD-Net은 **앵커 모양 비율 매칭**이다. GT 상자의 너비·높이가 앵커의 4배 이내면 매칭되고,
  매칭된 각 앵커마다 중심 셀과 이웃 셀 두 개가 양성이 된다. 도구 상자(평균 202 × 116 px)는
  9개 앵커 중 여러 개와, 그것도 여러 레벨에서 매칭된다. 그림 11(b)에서 도구의 양성은
  P3/P4/P5에 22 / 45 / 34 %로 퍼져 있다. 반면 32 px 팁 상자는 **P3의 작은 앵커에만** 매칭되어
  100 %가 한 레벨에 갇힌다.
- YOLOv8과 YOLO26의 TaskAlignedAssigner는 GT마다 정렬도 상위 `topk = 10`개를 고른다.
  **상자 크기와 무관하게 정원이 같으므로** 두 클래스가 9.6개와 9.8〜9.9개로 균등하다.
  YOLO26이 더한 상자 크기 하한(§2.2)은 32 px 팁 상자에 걸리지 않으므로 이 측정에서
  YOLOv8s과의 차이를 만들지 않는다.

정적 규칙은 "상자가 크면 더 많은 앵커와 레벨에 걸린다"는 기하학적 사실을 그대로 학습 신호의
불균형으로 옮긴다. 동적 할당은 그 결합을 끊는다. **§3.1의 비대칭, 즉 `tool`은 거의 대등한데
`tip`만 크게 벌어지는 현상이 이 1.81 대 0.97〜0.98의 차이와 방향이 일치한다.**

그림 11(b)는 레벨 분포에서 세 모델이 또 갈리는 것을 보여 준다. `tip`은 세 모델 모두 P3에
96〜100 % 몰리지만, `tool`에서 YOLOv8s은 P4에 81 %를 몰아 넣는 반면 YOLO26은 P4/P5에
49 / 40 %로 나누고 CLAD-Net은 P3/P4/P5에 22 / 45 / 34 %로 퍼뜨린다. **동적 할당끼리도
어느 해상도에서 배우는지는 다르다**는 뜻이지만, 이 차이가 성능에 어떻게 연결되는지는
이 실험으로 분리되지 않는다.

### 4.2 신뢰도 목표값의 구조

두 번째 차이는 신뢰도를 무엇에 맞춰 학습하는가다. 이 항목은 CLAD-Net과 나머지 둘 사이의
차이이며, YOLOv8s과 YOLO26은 같은 구조를 쓴다.

- CLAD-Net(YOLOv5 계열)은 objectness의 학습 목표를 예측·GT 상자의 **절대 CIoU**로 둔다.
  작은 상자에서는 이 값이 구조적으로 낮게 묶인다. 32 px 팁 상자는 640 공간에서 27.8 px이라
  중심이 2 px 어긋나면 IoU가 0.87, 4 px이면 0.75다.
- YOLOv8과 YOLO26은 objectness가 없고 클래스 점수가 곧 신뢰도이며, 그 목표값은
  TaskAlignedAssigner가 **GT마다 재정규화한 정렬도**다. 각 GT의 최고 정렬 앵커가 그 GT에서
  달성 가능한 IoU를 목표로 받으므로 절대 IoU에 묶이지 않는다.

측정된 신뢰도가 이 구조 차이와 일치한다.

표 12. 팁 예측 신뢰도 비교 (cholec80, `conf=0.25` 평가)

| 지표                            | CLAD-Net | YOLOv8s | YOLOv8s(참조) |    YOLO26 | YOLO26(참조) |
| ------------------------------- | -------: | ------: | ------------: | --------: | -----------: |
| 팁 예측 최대 신뢰도             |    0.925 |   0.909 |       *0.928* | **0.936** |      *0.948* |
| 10 px 이내 예측의 신뢰도 중앙값 |    0.580 |   0.660 |       *0.710* | **0.741** |      *0.792* |

팁 상자를 32 px로 키운 뒤로는 **다섯 회차 모두 천장이 사라졌다**(최대 0.9 이상). 절대 IoU에
묶이는 구조가 여전히 CLAD-Net에 불리하게 작용하지만(중앙값 0.580), 10 px 시절만큼
결정적이지는 않다. 즉 **이 요인은 팁 상자 크기 선택으로 이미 대부분 완화됐다.**

사전학습 회차는 같은 아키텍처보다 신뢰도가 한 단계 더 높다(0.660 → 0.710, 0.741 → 0.792).
잘 맞힌 예측에 더 확신을 싣는다는 뜻이지만, §3.3에서 본 것처럼 **정밀도로는 이어지지 않았다.**

**YOLO26의 신뢰도가 가장 높다**(중앙값 0.741). end-to-end 헤드는 객체당 상자를 하나만
남기므로 중복 상자를 걸러 내던 NMS의 일을 점수 자체가 떠안는데, 그 압력이 정확히 맞힌
예측의 점수를 밀어 올린 것으로 읽힌다. 다만 §3.3에서 본 것처럼 **그 높은 신뢰도가 오탐
감소로는 이어지지 않았다.**

### 4.3 후보 수와 오탐

CLAD-Net은 셀마다 앵커 3개를 예측하므로 프레임당 후보가 25,200개이고, 나머지 둘은
앵커프리라 8,400개다. 낮은 임계값에서 실제로 살아남는 예측 수도 그만큼 차이 난다.

표 13. 낮은 신뢰도 임계값에서의 클래스별 예측 수 (cholec80, `conf=0.001`)

| 클래스         |  CLAD-Net |   YOLOv8s | YOLOv8s(참조) |    YOLO26 | YOLO26(참조) |
| -------------- | --------: | --------: | ------------: | --------: | -----------: |
| `tool` 예측 수 | 1,871,421 |   639,771 |     *672,693* |   706,934 |    *956,498* |
| `tip` 예측 수  | 3,684,172 | 1,904,849 |   *1,729,614* | 2,625,878 |  *3,127,433* |

후보가 3배면 NMS 비용도, 임계값을 통과하는 오탐도 늘어난다. §3.3의 FP 격차(46,641 대
30,694)와 §3.5의 속도 격차가 같은 뿌리에서 나온다.

**YOLO26의 예측 수가 YOLOv8s보다 38 % 많다는 것은 뜻밖이다.** 앵커 격자가 같고 헤드가
객체당 상자 하나를 내도록 학습된 모델인데, NMS 대신 `max_det`으로만 자르므로 낮은 점수의
상자가 그대로 남는다. §3.3의 FP 17 % 증가와 §3.1의 AP@0.5 열세가 여기서 설명된다.
**중복을 지우는 단계를 없앤 대가를 점수만으로는 완전히 메우지 못했다.**

참조 구현 회차가 이 해석을 뒷받침한다. NMS를 도는 YOLOv8s(참조)는 사전학습을 하고도 `tip`
예측이 오히려 **줄지만**(1,904,849 → 1,729,614), NMS가 없는 YOLO26(참조)는 크게 **늘어난다**
(2,625,878 → 3,127,433). 학습이 좋아져도 NMS가 있는 쪽은 후보가 정리되는 반면 없는 쪽은
낮은 점수의 상자가 그대로 쌓이는 것으로 보인다. 쌍이 둘뿐이라 단정할 수는 없다.

### 4.4 사전학습과 참조 구현이 벌어 주는 폭

같은 아키텍처를 두 번씩 학습했으므로, "구현체와 초기 가중치를 바꾸면 얼마나 달라지는가"를
직접 뺄 수 있다. **다만 §2.4에서 적었듯 사전학습·증강 레시피·구현체가 함께 바뀌므로, 아래
차이는 그 셋을 합친 값이다.**

표 13-1. 참조 구현(사전학습) 빼기 재구현(스크래치)

| 데이터셋   | 아키텍처 |  Δ mAP@0.5 | Δ tip AP@0.5 | Δ 탐지 실패율 |  Δ Hit@10 px | 최고 에포크  |
| ---------- | -------- | ---------: | -----------: | ------------: | -----------: | ------------ |
| `cholec80` | YOLOv8s  |     +0.004 |       +0.005 |  **−2.48 pp** | **+3.03 pp** | 108 → **62** |
| `cholec80` | YOLO26   | **+0.010** |   **+0.011** |  **−2.26 pp** | **+2.30 pp** | 107 → **64** |
| `erop`     | YOLOv8s  |     +0.007 |       +0.007 |      −0.48 pp |     +0.94 pp | 125 → 110    |
| `erop`     | YOLO26   |     +0.003 |       +0.003 |      −0.31 pp |     +0.35 pp | 123 → 103    |

**세 가지를 읽을 수 있다.**

1. **아키텍처 순위는 바뀌지 않는다.** 네 쌍 모두 개선폭이 mAP@0.5 0.010 이내인데, CLAD-Net과
   YOLOv8s의 아키텍처 격차는 0.054〜0.070이다. **다섯 배 이상 차이가 나므로 §3.1의 순위는
   구현체나 초기 가중치를 바꿔서 뒤집을 수 있는 크기가 아니다.** 재구현이 각 아키텍처를
   과소평가하지 않았다는 근거이기도 하다.
2. **얻는 것은 정확도가 아니라 회수율과 수렴 속도다.** AP는 소수점 셋째 자리에서 움직이는데
   탐지 실패율은 cholec80에서 2.3〜2.5 pp 내려가고 최고 에포크는 절반 가까이 당겨진다.
   사전학습된 백본이 "이미 아는 것"은 팁 좌표를 더 정밀하게 찍는 능력이 아니라 **도구가
   거기 있다는 사실을 놓치지 않는 능력**에 가깝다.
3. **그 이점은 분할 방식에 달려 있다.** 영상 단위로 나눈 `cholec80`에서는 실패율이 2 pp 넘게
   내려가지만, 인접 프레임이 train과 test에 함께 들어가는 `erop`에서는 0.3〜0.5 pp에 그친다
   (§2.5). **시간적 누수가 있는 분할은 사전학습의 값어치까지 가린다.**

### 4.5 tool 감독 신호의 기여: 팁만 레이블링하면 어떻게 되는가

여기까지의 다섯 회차는 모두 `tool`과 `tip`을 **한 탐지기로 함께** 학습했다. 그 전제를 빼면
어떻게 되는지를 따로 물었다. 실무적으로는 어노테이션 비용의 문제다. 도구 상자를 그리는
일이 팁 좌표를 찍는 일보다 비싸므로, **팁만 레이블링해도 팁 탐지가 유지된다면** 그 비용을
지불할 이유가 없다.

주 프로젝트는 이미 같은 축의 질문에 답을 갖고 있다. 히트맵 모델에서 분할 마스크를 쓰는
`gradient-seg`와 팁 좌표만 쓰는 `gaussian-tip`을 비교해 "마스크 레이블링 비용이 정확도로
회수되지 않는다"는 결론을 얻었다 [3]. 이 절은 **탐지 계열에서 상자 레이블에 대해 같은
질문**을 던진다.

#### 설계

두 학습 모드를 정의한다. 이름이 그대로 실행 옵션의 값이자 산출물 디렉터리 이름이다.

표 13-2. 두 학습 모드

| 모드      | 학습 클래스          | 산출물 경로                                             |
| --------- | -------------------- | ------------------------------------------------------- |
| `tooltip` | `tool` + `tip` (2개) | `baseline/<name>/data/model/<dataset>/tooltip/` (기본값) |
| `tiponly` | `tip` (1개)          | `baseline/<name>/data/model/<dataset>/tiponly/`          |

`tiponly`는 `tool` 상자를 라벨 생성 단계에서 **아예 만들지 않고** 탐지 헤드의 클래스 채널도
1개로 줄인다. "팁만 레이블링했다면 무엇을 학습했을 것인가"에 그대로 대응하는 설정이며,
GT 상자 수가 절반이 된다.

재구현 세 회차 × 데이터셋 둘 = **여섯 셀**을 `tiponly`로 다시 학습했다. 참조 구현 둘은
사전학습 가중치를 쓰고 데이터셋도 따로 변환해야 하므로(§2.4) 이 비교에서 제외했다. 세
재구현은 `--label-set`을 제외한 나머지 인수가 §2.4의 프로토콜과 **전부 같으므로 바뀌는
변인이 학습 모드 하나뿐이다.** 클래스 채널이 하나 줄면서 파라미터가 미세하게 달라지지만
(CLAD-Net −873, YOLOv8s −387, YOLO26 −774개, 모두 0.02 % 미만) 관측될 차이를 모델 용량으로
설명할 수는 없는 크기다.

**판정 기준은 결과를 보기 전에 확정했다.** 주 지표는 Hit@10 px, 보조는 탐지 실패율과
`tip` AP@0.5다. 여섯 셀 모두 Hit@10 px 차이가 ±1 pp 이내이거나 방향이 엇갈리면 "차이 없음",
다섯 셀 이상에서 같은 방향으로 2 pp 이상 벌어지면 "유의미", 그 사이면 아키텍처별로 나누어
서술하고 결론을 유보한다. **조합당 한 번씩만 학습했으므로 판정 단위는 개별 셀이 아니라
여섯 셀에 걸친 방향의 일관성이다.**

#### 결과

![그림 12. 학습 모드 대비. (a) 주 판정 지표 Hit@10 px, (b) 세 지표의 차이를 오른쪽일수록 tiponly가 좋도록 부호를 맞춰 모은 것. 회색 띠가 판정 기준의 ±1 pp 무차별 구간이다. CLAD-Net만 반대쪽으로 간다.](figures/baseline-report-label-set.png)

표 13-3. `tiponly` 빼기 `tooltip` (test 전수, 팁 지표는 같은 정의라 그대로 비교된다)

| 데이터셋   | 모델     | Hit@10 px [pp] | 탐지 실패율 [pp] | `tip` AP@0.5 | 오차 중앙값 [px] | 헝가리안 정밀도 |
| ---------- | -------- | -------------: | ---------------: | -----------: | ---------------: | --------------: |
| `cholec80` | CLAD-Net |     **+5.34**  |       **−2.97**  |  **+0.065**  | 4.93 → **3.73**  | 0.663 → 0.687   |
| `cholec80` | YOLOv8s  |          −0.16 |            −0.79 |       −0.023 |  3.60 → 3.89     | 0.760 → 0.725   |
| `cholec80` | YOLO26   |          −1.40 |            +0.65 |       −0.014 |  3.45 → 3.67     | 0.728 → 0.729   |
| `erop`     | CLAD-Net |     **+5.60**  |       **−1.48**  |  **+0.063**  | 3.23 → **2.22**  | 0.714 → 0.739   |
| `erop`     | YOLOv8s  |          −2.33 |            +1.16 |       −0.022 |  2.04 → 2.08     | 0.829 → 0.814   |
| `erop`     | YOLO26   |          −0.78 |            +0.53 |       −0.007 |  1.96 → 1.97     | 0.818 → 0.820   |

> 각 칸은 그대로 뺀 값이다. **탐지 실패율만 음수가 개선**이고 나머지는 양수가 개선이다.
> 오차 중앙값과 헝가리안 정밀도는 차이 대신 `tooltip` → `tiponly` 값을 적었다.

**판정은 세 번째 구간이다.** 여섯 셀이 ±1 pp 안에 들지도 않았고, 다섯 셀 이상이 같은 방향으로
2 pp 이상 벌어지지도 않았다. 갈리는 선은 데이터셋이 아니라 **아키텍처**다.

- **CLAD-Net은 `tiponly`가 확연히 낫다.** 두 데이터셋 모두 Hit@10 px가 5 pp 넘게 오르고,
  탐지 실패율·오차 중앙값·`tip` AP·헝가리안 정밀도가 **함께** 좋아진다. 어느 하나를 다른
  하나와 맞바꾼 것이 아니다.
- **앵커프리 두 모델은 `tiponly`가 조금 못하다.** 여섯 셀 중 네 셀에서 Hit@10 px가
  0.16〜2.33 pp 내려간다. 폭이 작고 지표마다 방향이 완전히 일치하지도 않는다
  (YOLOv8s / `cholec80`은 Hit@10 px가 −0.16 pp인데 탐지 실패율은 0.79 pp 좋아진다).

#### 왜 CLAD-Net만 반대인가

설계 단계에서 미리 적어 둔 의심 항목이 있었다. `common/model.py`의 앵커는 `tool` 상자와
`tip` 상자를 **함께** k-means하여 얻은 값인데, `tiponly`에서도 그대로 두었다. 도구 크기에
맞춰진 큰 앵커가 양성을 하나도 받지 못하므로 **유효 용량이 줄어드는 불리함이 CLAD-Net에만
있을 것**이라고 보았다. 관측된 방향은 그 예상과 정반대다.

§4.1의 할당 진단을 두 모드에 각각 한 번씩 더 돌려 확인했다. 학습은 하지 않고, `cholec80`
val 400 프레임에서 "GT 상자 하나가 몇 개의 양성 앵커를 받는가"를 세는 같은 측정이다.

표 13-4. 학습 모드별 GT당 양성 할당 수 (cholec80 val 400 프레임)

| 모델     | `tooltip`의 `tool` | `tooltip`의 `tip` | tool : tip | `tiponly`의 `tip` |
| -------- | -----------------: | ----------------: | ---------: | ----------------: |
| CLAD-Net |               16.2 |          **8.98** |   **1.81** |          **8.98** |
| YOLOv8s  |                9.6 |              9.82 |       0.98 |              9.92 |
| YOLO26   |                9.6 |              9.86 |       0.97 |              9.94 |

**팁 GT가 받는 학습 신호는 두 모드에서 똑같다.** CLAD-Net에서 8.98개로 소수점 둘째 자리까지
같고, 레벨 분포도 100 %가 P3에 몰리는 것까지 같다. 앵커 매칭이 GT 상자의 기하만 보고
클래스와 무관하게 정해지기 때문이다. **즉 "앵커를 다시 뽑아야 한다"는 의심은 기각된다.**
큰 앵커가 노는 것은 사실이지만, 그것이 팁의 학습 신호를 줄이지는 않는다.

바뀌는 것은 **`tool` 쪽이 통째로 사라진다는 것**이다. `tooltip`에서 CLAD-Net은 도구 GT
하나에 팁의 1.81배를 배정하고(표 11), 그 양성의 22 %가 **팁이 100 % 사는 바로 그 P3 레벨**에
놓인다. 팁 좌표는 언제나 도구 상자 안에 있으므로 두 클래스의 양성은 같은 격자에서 겹친다.
`tiponly`는 그 경쟁을 제거한다. §4.1이 CLAD-Net의 `tip` 격차 원인으로 지목한 **정적 할당의
클래스 불균형이 처치로 없어지는 셈**이고, 팁 지표가 오르는 방향과 일치한다.

동적 할당 두 모델에서는 얻을 것이 처음부터 없었다. `tooltip`에서 이미 0.97〜0.98로 균형이고,
`tiponly`에서 팁의 양성 수는 9.82 → 9.92, 9.86 → 9.94로 사실상 변하지 않는다. `topk = 10`이라
상자 크기에도, 함께 학습하는 클래스 수에도 정원이 흔들리지 않기 때문이다. 남는 것은
`tool` 상자가 주던 보조 감독을 잃는 쪽뿐이고, 소폭 하락이 그와 방향이 같다.

**이 설명은 정황 증거다.** CLAD-Net의 앵커 매칭만 가중치와 무관하게 결정되고, 나머지 둘의
정렬도 기반 할당은 측정에 쓴 체크포인트에 의존한다. 두 모드의 체크포인트가 다르므로 그
차이가 섞여 있다.

#### 부수적으로 관찰된 것

- **신뢰도는 세 모델 모두 오른다.** 10 px 이내로 맞힌 예측의 신뢰도 중앙값이 CLAD-Net
  0.580 → 0.676, YOLOv8s 0.660 → 0.727, YOLO26 0.741 → 0.769이다. 클래스가 하나뿐이면
  분류가 쉬워지므로 예상되는 방향이다. §4.2에서 본 절대 IoU 천장은 그대로다.
- **§6.1의 과소 탐지는 꿈쩍하지 않는다.** 큰 오차(≥ 200 px) 중 예측 공유가 차지하는 비율이
  CLAD-Net 82.1 → 82.2 %, YOLOv8s 88.5 → 86.2 %, YOLO26 87.2 → 87.7 %로 사실상 같다.
  **한 프레임의 도구 하나를 통째로 놓치는 실패 모드는 `tool` 감독 신호와 무관하다.**
- **학습 시간은 거의 같다.** GT 상자 수가 절반이어도 프레임 수와 순전파 비용이 같아,
  여섯 셀 모두 `tooltip` 대비 −0.5 〜 +0.3 시간 안에 든다.

#### 실무적 결론

**"팁만 레이블링해도 된다"고 일반화할 수는 없다.** 방향이 아키텍처에 따라 갈렸기 때문이다.
다만 두 가지는 말할 수 있다.

1. **잃는 폭은 어느 셀에서도 작다.** 가장 나쁜 셀이 Hit@10 px −2.33 pp(YOLOv8s / `erop`)이며,
   같은 지표에서 아키텍처 선택이 만드는 격차(§3.2의 CLAD-Net과 YOLOv8s 차이 3.2〜7.4 pp)보다
   작다. **상자 어노테이션을 절반으로 줄이는 대가로는 싼 편이다.**
2. **정적 할당을 쓰는 탐지기라면 오히려 `tiponly`가 낫다.** CLAD-Net에서는 비용을 줄이면서
   성능도 얻는다. 반대로 동적 할당을 쓰는 두 모델에서는 `tool` 상자가 소폭의 값을 한다.

주 프로젝트의 히트맵 실험과 나란히 놓으면 하나의 그림이 된다. **팁 좌표 외의 부가 레이블
(분할 마스크든 도구 상자든)이 팁 탐지 정확도로 회수되는 폭은, 지불하는 레이블링 비용에
비해 작다.** 다만 이 실험은 조합당 단일 실행이므로 1 pp 안팎의 셀 하나를 근거로 삼지 않았고,
그래서 판정도 유보에 두었다.

### 4.6 정리: 무엇이 얼마나 기여했는가

표 14. 성능 및 속도 격차에 기여하는 요인의 종합

| 요인                               | 근거                                            | 어느 비교에 걸리는가  | `tip` 격차 기여                 | 속도 격차 기여                |
| ---------------------------------- | ----------------------------------------------- | --------------------- | ------------------------------- | ----------------------------- |
| **정적 할당의 클래스 불균형**      | 직접 측정 (그림 11), 1.81 대 0.97〜0.98         | CLAD-Net 대 나머지 둘 | **크다**                        | 없음                          |
| 후보 수 3배 (앵커 3개 대 앵커프리) | 예측 수 2.9배, FP 1.5배                         | CLAD-Net 대 나머지 둘 | 중간 (오탐)                     | **크다**                      |
| CAM의 연산 비용                    | P3에서 80 × 80 × 336 텐서에 풀링·업샘플 4회     | CLAD-Net 대 나머지 둘 | 없음                            | **크다**                      |
| objectness 목표값 = 절대 IoU       | 신뢰도 중앙값 0.580 대 0.660〜0.741             | CLAD-Net 대 나머지 둘 | 작다 (32 px에서 완화됨)         | 없음                          |
| **DFL 유무**                       | 오차 중앙값 3.45(없음) 대 3.60(있음) 대 4.93 px | YOLO26 대 YOLOv8s     | 중간 (미분리)                   | 없음                          |
| **NMS 제거 (end-to-end 헤드)**     | FP 35,877 대 30,694, 예측 수 +38 %              | YOLO26 대 YOLOv8s     | 중간 (불리하게)                 | 없음                          |
| C3k2 + C2PSA 백본                  | 백본 + 넥만 6.09 ms (C2PSA는 0.60 ms)           | YOLO26 대 YOLOv8s     | 없음                            | 중간                          |
| MuSGD 옵티마이저                   | **미분리** (§2.4)                               | YOLO26 대 나머지 둘   | 미지                            | 없음                          |
| 사전학습 + 증강 + 구현체 (묶음)    | 직접 측정 (표 13-1), mAP@0.5 +0.003〜0.010      | 참조 구현 대 재구현   | 작다 (아키텍처 격차의 1/5 이하) | 중간 (예측 래퍼 +1.9〜2.3 ms) |
| **`tool` 감독 신호의 유무**        | 직접 측정 (표 13-3·13-4), Hit@10 px −2.3〜+5.6 pp | `tooltip` 대 `tiponly` | 아키텍처마다 **부호가 다르다**  | 없음                          |

**직접 측정으로 인과를 짚은 것은 첫 두 항목과 백본 비용, 그리고 마지막 두 항목뿐이다.** DFL 유무, end-to-end 헤드,
C3k2 블록의 기여는 이 실험 설계로 분리되지 않는다. 특히 **YOLO26의 결과에는 옵티마이저
차이(MuSGD)가 섞여 있어**, YOLOv8s과의 차이를 곧바로 "아키텍처의 성질"로 읽으면 안 된다.
분리하려면 헤드만 교체하거나 옵티마이저만 바꾸는 통제된 어블레이션이 필요하다.
본 보고서의 인과 주장은 **정황 증거 수준**임을 명시한다.

## 5. 세그멘테이션 기반 방법과의 비교

주 프로젝트의 히트맵 회귀 방식 [3] 과는 성향이 뚜렷이 갈린다. 그림 5의 두 곡선이 교차하는
지점이 그 차이를 요약한다.

표 15. 객체 탐지 기반과 세그멘테이션 기반 팁 추정의 비교 (cholec80, 탐지 기반은 최고값)

괄호 안은 참조 구현까지 넣었을 때의 최고값이다.

|                           | 탐지 기반 (본 보고서)              | 세그멘테이션 기반 [3] |
| ------------------------- | ---------------------------------- | --------------------- |
| 출력                      | 상자 + 클래스 → 상자 중심이 팁     | 히트맵 → 피크가 팁    |
| 근거리 정확도 (Hit@10 px) | **59.9 %** (YOLOv8s, 참조 62.9 %)  | 59.6 % / 30.0 %       |
| 오차 중앙값               | **3.45 px** (YOLO26, 참조 3.31 px) | 5.39 px / 16.12 px    |
| 원거리 회수 (Hit@50 px)   | 71.8 % (YOLOv8s, 참조 75.8 %)      | 74.7 % / **76.1 %**   |
| 탐지 실패율               | 8.82 % (YOLOv8s, 참조 6.34 %)      | 7.48 % / **2.90 %**   |
| 부가 출력                 | **도구 상자도 함께**               | 팁 좌표만             |
| 속도                      | **4.89 ms** (YOLOv8s)              | 11.9 ms               |

사전학습을 넣어도 이 성향은 바뀌지 않는다. 가장 좋은 탐지 회차조차 탐지 실패율 6.34 %로
`gradient-seg`의 2.90 %에 두 배 넘게 뒤지고, 대신 Hit@10 px는 두 배 이상 앞선다.

**탐지기는 "정확히 짚거나 아예 없거나", 히트맵은 "대충이라도 어딘가는 짚는다".**
`gradient-seg`는 탐지 실패율이 2.90 %로 압도적으로 낮지만 Hit@10 px가 30.0 %에 그친다.
피크 추출이 임계값을 넘는 영역마다 후보를 만들어 내므로 놓치는 일은 드물지만, 그만큼
정밀하지 않다.

**응용이 선택을 정한다.** 로봇 제어나 계측처럼 정밀 좌표가 필요하면 탐지 기반이,
카메라 자동 유도처럼 대략적 방향만 필요하면 놓침이 적은 히트맵 기반이 낫다. 도구 상자가
함께 필요하거나 실시간성이 빠듯하다면 탐지 기반이 유일한 선택이다.

## 6. 한계 (Limitations)

### 6.1 공통 한계: 다중 도구 프레임의 과소 탐지

탐지 다섯 회차와 히트맵 회귀 모두가 같은 실패 모드를 안고 있다. 큰 오차(≥ 200 px)의 정체를 `per_tip.csv`에서
확인하면, 대부분은 위치를 틀린 것이 아니라 **한 프레임의 도구 하나를 통째로 놓쳐** 그
GT 팁이 다른 도구의 예측에 매칭된 경우다.

표 16. 큰 팁 오차에서 예측 공유가 차지하는 비율

| 모델 (cholec80) | ≥ 200 px 오차 | 그중 예측 공유 |       비율 |
| --------------- | ------------: | -------------: | ---------: |
| YOLOv8s         |        12,629 |         11,175 | **88.5 %** |
| YOLOv8s(참조)   |      *11,106* |        *9,510* |   *85.6 %* |
| YOLO26          |        13,139 |         11,454 | **87.2 %** |
| YOLO26(참조)    |      *12,558* |       *10,724* |   *85.4 %* |
| CLAD-Net        |        12,875 |         10,568 | **82.1 %** |

프레임 내 도구 수별 탐지 실패율도 같은 방향을 가리킨다. 다섯 회차 모두 **도구가 하나뿐인
프레임**에서 가장 많이 놓친다(YOLOv8s 18.1 %, YOLO26 19.3 %, CLAD-Net 19.6 %; 도구 2개
프레임은 6.0 %, 6.9 %, 6.9 %). 기구가 화면에 막 들어오거나 나가는 장면, 일부만 보이는 장면이
여기 몰리는 것으로 보인다.

**NMS를 없앤 YOLO26에서도 이 비율이 87.2 %로 그대로다.** 중복 상자를 지우는 방식의 문제가
아니라, 한 프레임에 비슷하게 생긴 도구가 둘 이상 있을 때 그중 하나만 높은 점수를 받는
문제라는 뜻이다.

**`tool` 감독 신호를 빼도 이 비율은 그대로다.** 팁만 학습한 여섯 회차에서도 예측 공유가
차지하는 비율은 82.2 %·86.2 %·87.7 %로 사실상 같다(§4.5). 도구 상자를 함께 학습하든 말든
바뀌지 않는 실패 모드라는 뜻이다.

**사전학습도 이 실패 모드를 지우지 못한다.** 참조 구현 회차에서 큰 오차의 절대 수는 줄지만
(12,629 → 11,106, 13,139 → 12,558) 그중 예측 공유가 차지하는 비율은 85 %대로 거의 그대로다.
도구 하나뿐인 프레임의 실패율도 13.6 %·15.8 %로 내려갈 뿐 여전히 도구 2개 프레임
(4.2 %·5.2 %)의 세 배다. **이것이 남은 가장 큰 개선 여지이며, 헤드 설계로도 사전학습으로도
해결되지 않았다.**

### 6.2 방법론 및 실험 한계

- **CLAD-Net 재구현의 동치성을 보장할 수 없다.** 저자 공개 코드가 없어 논문 해석에 의존했고,
  검증 수단은 파라미터 수 일치뿐이다. 본 보고서의 CLAD-Net 성능은 **이 재구현의 성능**이며
  원 논문 방법의 성능이라 단정할 수 없다.
- **인과 분석이 정황 증거다.** §4.1의 할당 밀도는 직접 측정이지만, 그것이 최종 AP 격차의 몇
  퍼센트를 설명하는지는 통제된 어블레이션 없이는 말할 수 없다.
- **`erop`은 시간적 누수가 있다.** 프레임 단위 분할이라 낙관적이다.
- **논문 수치와 비교 불가.** 논문의 Cholec80-sub는 비공개이고, 8:1:1 무작위 분할이라 test에
  train과 거의 같은 프레임이 들어간다. AP@0.5 98.9 %는 상당 부분 그 분할 방식에서 온 것으로 보인다.
- **단일 시드.** 각 조합을 한 번씩만 학습했으므로 분산을 알 수 없다. §4.5의 여섯 셀도
  마찬가지여서, 그 절의 판정을 셀 하나가 아니라 **방향의 일관성**으로 내린 것이 그에 대한
  대응이다. 1 pp 안팎의 차이는 실행 간 변동과 구분되지 않는다.
- **§4.5의 `tiponly` 비교에서 `tool` AP와 전체 mAP는 정의되지 않는다.** 그 회차에는 `tool`
  클래스가 없으므로 팁 지표로만 견주었다. 도구 상자가 필요한 응용이라면 `tiponly`는 애초에
  선택지가 아니다.
- **CLAD-Net의 앵커를 다시 뽑지 않았다.** `tiponly`에서도 `tool` 상자까지 함께 k-means한
  앵커를 그대로 썼다. 표 13-4가 팁의 학습 신호는 그대로임을 보였으므로 결론을 뒤집을 항목은
  아니지만, 팁 전용 앵커로 다시 뽑으면 CLAD-Net의 `tiponly`가 더 오를 여지는 남아 있다.
- **팁 상자 32 px이 최적이라는 근거는 없다.** 유도된 값이지 탐색된 값이 아니다.
- **YOLO26의 비교에는 옵티마이저 차이가 섞여 있다.** 재구현 세 회차 중 이 회차만 MuSGD로 학습했다
  (§2.4). YOLOv8s과의 차이를 아키텍처 몫으로만 읽으면 안 되며, `--optimizer sgd` 실행을
  한 회차 쌓아야 분리된다.
- **`conf` 임계값을 탐색하지 않았다.** 다섯 회차 모두 `conf=0.25`를 썼다. YOLO26은 정밀도가
  강점이고 재현율이 약점인 쪽에 있으므로, 임계값을 낮췄을 때 얻을 것이 가장 많은 모델이다.
- **참조 구현 대조군은 조건이 하나만 다른 것이 아니다.** 초기 가중치·증강 레시피·구현체가
  함께 바뀌므로(표 4-1), §4.4의 차이를 "사전학습의 효과"라고 부를 수 없다. 셋을 나누려면
  `--no-pretrained` 회차가 필요하고 아직 돌리지 않았다.

## 7. 향후 연구 (Future Work)

1. **과소 탐지 대응.** 큰 오차의 82〜88 %를 차지하고, NMS를 없애도 줄지 않았다. TAL `topk`
   상향(YOLO26은 one2one 분기의 `topk`가 7이다), 팁 클래스 전용 임계값, NMS IoU 조정을
   시험할 수 있다.
2. **통제된 어블레이션.** CLAD-Net의 헤드·할당기만 YOLOv8식으로 교체해 §4의 요인을 분리하고,
   YOLO26을 `--optimizer sgd`로 한 회차 더 학습해 옵티마이저 몫을 떼어 낸다.
   참조 구현 둘은 `--no-pretrained`로 한 회차씩 더 학습하면 §4.4의 묶음에서 **사전학습의
   몫만** 떼어 낼 수 있다. 명령은 두 프로젝트의 `docs/commands.md`에 이미 준비돼 있다.
3. **팁 상자 크기 탐색.** 24 / 32 / 48 px를 비교한다.
4. **입력 캔버스 모양.** 640 × 640 letterbox는 34.8 %가 패딩이다. 800 × 512로 바꾸면 같은
   연산량에서 물체가 23 % 커지고 objectness 기울기가 1.53배가 된다
   ([CLAD-Net 보고서 부록 B](../baseline/cladnet/docs/experimental-results.md)).
5. **CLAD-Net 장기 학습.** 150 에포크에서 아직 포화하지 않았다. 반대로 나머지 넷은
   과하다. 사전학습 회차는 cholec80에서 62〜64 에포크면 끝나므로(§3.4), 에포크 예산을
   아키텍처별로 다시 잡는 것만으로 실험 회전이 빨라진다.
6. **실행 경로 최적화.** §3.5의 (A)와 (B) 격차가 보여 주듯 `ultralytics`의 예측 래퍼는
   프레임당 2 ms 가까이 쓴다. 배치 추론이나 전처리의 GPU 이관으로 회수할 수 있는 몫이다.
7. **팁 전용 앵커.** §4.5에서 CLAD-Net만 `tiponly`가 나은 이유를 정적 할당의 클래스 불균형
   해소로 읽었다. 32 px 팁 상자만으로 k-means를 다시 돌려 앵커 아홉 개를 뽑으면, 노는 큰
   앵커를 없애 남은 여지를 확인할 수 있다. 앵커가 바뀌면 변인이 둘이 되므로 별도 회차로
   돌려야 한다.
8. **`tiponly` 반복 실행.** §4.5의 앵커프리 두 모델은 차이가 0.16〜2.33 pp로 작아 단일
   실행과 구분되지 않는다. 시드를 바꿔 두세 번씩 더 돌리면 "`tool` 상자가 소폭의 값을
   한다"는 서술을 확정하거나 버릴 수 있다.

## 8. 결론 (Conclusion)

동일 조건에서 **두 범용 탐지기가 전용 아키텍처를 앞섰다.** cholec80 test mAP@0.5는
YOLOv8s 0.751, YOLO26 0.725, CLAD-Net 0.697이고, 속도는 4.89 / 6.72 / 8.78 ms로 **파라미터가
가장 적은 CLAD-Net이 가장 느렸다.** 내시경 수술도구 탐지를 위해 설계된 전용 아키텍처가
범용 탐지기의 두 세대에 모두 뒤진 셈이다.

**이 결론은 구현체와 초기 가중치에 기대지 않는다.** 같은 두 아키텍처를 `ultralytics` 참조
구현으로, COCO 사전학습에서 다시 학습한 대조군에서도 순위는 같았다(mAP@0.5 0.755 · 0.735 ·
0.697). 그 묶음이 같은 아키텍처 안에서 벌어 준 폭은 mAP@0.5 0.003〜0.010으로, 아키텍처
사이의 격차(0.054〜0.070)의 5분의 1에도 못 미친다. 대신 얻은 것은 **수렴 속도와 회수율**이다.
cholec80 최고 에포크가 절반으로 당겨지고(107〜108 → 62〜64) 탐지 실패율이 2.3〜2.5 pp
내려간다. 그마저도 시간적 누수가 있는 `erop` 분할에서는 0.3〜0.5 pp로 줄어든다.

CLAD-Net과의 격차는 `tip` 클래스에 몰렸고, 학습 없이 측정한 라벨 할당 통계가 그 원인을
가리킨다. CLAD-Net의 정적 앵커 비율 매칭은 큰 객체에 작은 객체의 두 배 가까운 학습 신호를
배정하는(1.81 : 1) 반면, 동적 할당을 쓰는 두 모델은 상자 크기와 무관하게 균등하다
(0.97〜0.98 : 1). **작은 객체를 다루는 과제에서 동적 라벨 할당의 이점이 크다**는 것이 이
실험의 주된 시사점이다.

**세대가 새로울수록 낫다는 결론은 나오지 않았다.** YOLO26은 YOLOv8s보다 한 세대 뒤에
나왔고 파라미터가 11 % 적지만, 이 과제에서 AP@0.5·탐지 실패율·오탐·속도가 모두 뒤진다.
얻은 것은 **엄격한 IoU와 팁 좌표 정밀도**다(mAP@0.5:0.95 0.555 대 0.552, 팁 오차 중앙값
3.45 대 3.60 px). DFL을 버리고 거리를 직접 회귀한 표현의 이점으로 읽힌다. 반면 **NMS를
없앤 end-to-end 헤드는 이 과제에서 이점을 보이지 못했다**: 오탐이 17 % 늘고 예측 수가
38 % 많아졌으며, 과소 탐지 비율(87.2 %)도 그대로였다. **정밀 좌표가 목적이면 YOLO26,
전반적 탐지 품질과 속도가 목적이면 YOLOv8s**가 이 데이터에서의 선택이다.

**부가 레이블의 값어치도 라벨 할당 방식이 가른다.** `tool` 상자 없이 팁만 학습한 여섯
회차를 더해 보니, **CLAD-Net은 팁만 학습한 쪽이 오히려 낫고**(Hit@10 px +5.34·+5.60 pp)
앵커프리 두 모델은 조금 못하다(−0.16〜−2.33 pp). 학습 없이 할당을 다시 재어 원인을 좁혔다.
팁 GT가 받는 양성 앵커 수는 두 모드에서 소수점 둘째 자리까지 같고(CLAD-Net 8.98개), 빠지는
것은 팁의 1.81배를 가져가며 같은 P3 격자에서 겹치던 `tool` 쪽 신호다. **정적 할당의 클래스
불균형이 처치로 사라지는 셈이다.** 잃는 폭은 가장 나쁜 셀에서도 2.33 pp로 아키텍처 선택이
만드는 격차(3.2〜7.4 pp)보다 작으므로, **상자 어노테이션을 절반으로 줄이는 대가로는 싸다.**
다만 방향이 아키텍처마다 갈렸고 조합당 단일 실행이므로 "팁만 레이블링해도 된다"고
일반화하지는 않는다(§4.5).

**속도는 아키텍처가 아니라 실행 경로가 가른다.** 같은 아키텍처를 같은 GPU에서 돌려도,
네트워크와 후처리만으로는 참조 구현이 대등하거나 빠른데 프레임 한 장을 상자로 바꾸는
전 경로에서는 1.20〜1.28배 느리다. `ultralytics`의 예측 래퍼가 프레임당 1.85〜2.32 ms를
쓰기 때문이며, 재구현의 letterbox와 장치 전송은 0.52〜0.64 ms로 끝난다.

세그멘테이션 기반 접근과 비교하면 탐지 기반은 **근거리 정밀도와 속도**에서, 히트맵 기반은
**놓치지 않는 성질**에서 앞선다. 일곱 회차 모두 다중 도구 프레임의 과소 탐지라는 같은 한계를
공유하며, 사전학습으로도 지워지지 않았다(§6.1).

## 참고문헌 (References)

[1] X. Zhao, J. Guo, Z. He, X. Jiang, H. Lou, D. Li, "CLAD-Net: cross-layer aggregation
attention network for real-time endoscopic instrument detection," *Health Information Science
and Systems*, vol. 11, no. 1, art. 58, 2023. doi:10.1007/s13755-023-00260-9

[2] G. Jocher, A. Chaurasia, J. Qiu, "Ultralytics YOLOv8," 2023.
<https://github.com/ultralytics/ultralytics>

[3] 본 저장소 주 프로젝트, "복강경 수술 영상에서의 히트맵 회귀를 이용한 수술도구 팁 탐지,"
[`docs/final-report.md`](final-report.md).

[4] C. Feng, Y. Zhong, Y. Gao, M. R. Scott, W. Huang, "TOOD: Task-aligned One-stage Object
Detection," *ICCV*, 2021.

[5] O. M. Omisore, S. Han, J. Xiong, H. Li, Z. Li, L. Wang, "A review on flexible robotic
systems for minimally invasive surgery," *IEEE Trans. Systems, Man, and Cybernetics: Systems*,
vol. 52, no. 1, pp. 631–644, 2020.

[6] O. Ronneberger, P. Fischer, T. Brox, "U-Net: Convolutional Networks for Biomedical Image
Segmentation," *MICCAI*, 2015.

[7] G. Jocher et al., "YOLOv5," 2020. <https://github.com/ultralytics/yolov5>

[8] Z. Ge, S. Liu, F. Wang, Z. Li, J. Sun, "YOLOX: Exceeding YOLO Series in 2021,"
arXiv:2107.08430, 2021.

[9] C. Li et al., "YOLOv6: A Single-Stage Object Detection Framework for Industrial
Applications," arXiv:2209.02976, 2022.

[10] Y. Zhao et al., "DETRs Beat YOLOs on Real-time Object Detection," *CVPR*, 2024.

[11] X. Li et al., "Generalized Focal Loss: Learning Qualified and Distributed Bounding Boxes
for Dense Object Detection," *NeurIPS*, 2020.

[12] Z. Zheng, P. Wang, W. Liu, J. Li, R. Ye, D. Ren, "Distance-IoU Loss: Faster and Better
Learning for Bounding Box Regression," *AAAI*, 2020.

[13] A. P. Twinanda, S. Shehata, D. Mutter, J. Marescaux, M. de Mathelin, N. Padoy,
"EndoNet: A Deep Architecture for Recognition Tasks on Laparoscopic Videos," *IEEE Trans.
Medical Imaging*, vol. 36, no. 1, pp. 86–97, 2017.

[14] Ultralytics, "YOLO26," 2025. <https://docs.ultralytics.com/models/yolo26>

[15] A. Wang, H. Chen, L. Liu, K. Chen, Z. Lin, J. Han, G. Ding, "YOLOv10: Real-Time
End-to-End Object Detection," *NeurIPS*, 2024.

[16] K. Jordan, Y. Jin, V. Boza, J. You, F. Cesista, L. Newhouse, J. Bernstein,
"Muon: An optimizer for hidden layers in neural networks," 2024.

## 부록: 재현

```bash
# 학습 — 재구현 세 회차 (동일 조건, 스크래치)
./baseline/cladnet/run      train-model --dataset cholec80 --frame-stride 5 --val-frames 1500
./baseline/yolov8sclone/run train-model --dataset cholec80 --frame-stride 5 --val-frames 1500
./baseline/yolo26clone/run  train-model --dataset cholec80 --frame-stride 5 --val-frames 1500

# 학습 — 참조 구현 두 회차 (COCO 사전학습). 자체 .venv를 쓰고, 학습 전에 데이터셋을
# Ultralytics 형식으로 한 번 변환한다.
./baseline/yolov8s/run prepare-dataset --dataset cholec80
./baseline/yolov8s/run train-model     --dataset cholec80 --workers 12
./baseline/yolo26/run  prepare-dataset --dataset cholec80
./baseline/yolo26/run  train-model     --dataset cholec80 --workers 12

# 학습 — 팁만 레이블링한 여섯 회차 (§4.5). --label-set 말고는 위와 인수가 같다.
./baseline/cladnet/run      train-model --dataset cholec80 --label-set tiponly --frame-stride 5 --val-frames 1500
./baseline/yolov8sclone/run train-model --dataset cholec80 --label-set tiponly --frame-stride 5 --val-frames 1500
./baseline/yolo26clone/run  train-model --dataset cholec80 --label-set tiponly --frame-stride 5 --val-frames 1500

# 평가 (속도를 재려면 유휴 GPU를 지정한다)
./baseline/cladnet/run      eval-model --dataset cholec80 --split test
./baseline/yolov8sclone/run eval-model --dataset cholec80 --split test
./baseline/yolo26clone/run  eval-model --dataset cholec80 --split test
./baseline/yolov8s/run      eval-model --dataset cholec80 --split test
./baseline/yolo26/run       eval-model --dataset cholec80 --split test

./baseline/cladnet/run      eval-model --dataset cholec80 --label-set tiponly --split test
./baseline/yolov8sclone/run eval-model --dataset cholec80 --label-set tiponly --split test
./baseline/yolo26clone/run  eval-model --dataset cholec80 --label-set tiponly --split test

# 각 서브 프로젝트의 수치 표
./baseline/cladnet/run      generate-summary
./baseline/yolov8sclone/run generate-summary
./baseline/yolo26clone/run  generate-summary
./baseline/yolov8s/run      generate-summary
./baseline/yolo26/run       generate-summary

# 팁만 학습한 회차의 수치 표 (§4.5)
./baseline/cladnet/run      generate-summary --label-set tiponly --output docs/summary-results-tiponly.md
./baseline/yolov8sclone/run generate-summary --label-set tiponly --output docs/summary-results-tiponly.md
./baseline/yolo26clone/run  generate-summary --label-set tiponly --output docs/summary-results-tiponly.md

# YOLO26 재구현이 참조와 일치하는지 (§2.3)
./baseline/yolo26clone/run  verify-clone

# 실험 현황 한눈에 보기 (Baselines 탭)
./run dashboard

# 이 보고서의 그림
uv run jupyter nbconvert --to notebook --execute --inplace \
    notebook/baseline-report-graph.ipynb
```

표 10의 속도는 스크립트가 아니라 §3.5의 블록 인용에 적은 절차로 잰 보조 측정이다. 다시
재려면 유휴 GPU 하나를 잡고, 각 서브 프로젝트의 `common.inference.Detector`로 같은 프레임
60장을 같은 반복 수만큼 돌리면 된다.
