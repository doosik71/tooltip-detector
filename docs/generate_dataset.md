...
이미지 포맷은 항상 `.png`이며, 기존 파일 존재 여부를 체크해 skip하지 않는다. 즉, 같은 출력 디렉터리로 다시 실행하면 동일 파일명이 덮어써질 수 있다.
...
- 기존 출력 파일을 보호하거나 skip하는 옵션이 없다.
- 저장 포맷이 PNG로 고정되어 있다.
- 로그 출력이 `print()`와 `tqdm`에 묶여 있어 구조화된 로그에는 적합하지 않다.

### 확장 권장 사항

1. `--overwrite` 또는 `--skip-existing` 옵션을 추가해 재실행 동작을 명확히 제어
2. 재귀 입력 탐색 옵션 추가
3. JPEG 저장 품질 또는 출력 포맷 선택 옵션 추가
4. 전체 데이터셋 기준 split 또는 비디오 단위 split을 선택할 수 있게 확장
5. 긴 영상 성능 개선을 위해 프레임 seek 또는 샘플링 전략 최적화
