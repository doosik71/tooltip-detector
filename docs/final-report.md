...
따라서 **같은 영상의 시간적으로 인접한 프레임이 train과 test에 동시에 존재한다**. `erop`의 10프레임(약 0.3초) 간격, `cholec80`의 25프레임(1초) 간격에서 인접 프레임의 장면은 거의 동일하므로, 이 분할로 측정한 test 성능은 **미지의 새 영상에 대한 일반화 성능보다 낙관적**이다. 본 보고서의 모든 정확도 수치는 이 전제 위에서 해석되어야 한다(§8 한계). 분할 규칙은 병합된 tooltip-annotator 하위 프로젝트의 [scripts/generate_dataset.py](../tooltip-annotator/scripts/generate_dataset.py)에 있으며, create_split_rng()와 assign_splits()가 영상별 프레임 셔플과 60:20:20 배정을 수행한다. 따라서 이후 영상 단위 재분할은 저장소 내 코드로 구현·재현할 수 있다.
...
**그림 2. `gradient-seg` 타겟 생성 과정 — 이진 분할 마스크에서 거리 기반 히트맵까지.** 마스크의 연결요소를 라벨링하고(②), bbox와 겹치는 화소 수가 최대인 어노테이션에 각 연결요소를 배정한 뒤(③), 어노테이션별로 독립 정규화하여 거리 기반 히트맵을 만든다(④). 도구 하나가 여러 연결요소로 쪼개질 수 있으므로 정규화는 개별 CC가 아니라 그룹 단위로 수행된다. ④의 흰 점선(값 0.5 등고선)이 도구 몸통의 절반 지점에 서는 것이 §6.3에서 관찰되는 정밀도·miss율 대비의 직접적 원인이다.
...
**그림 4. `erop` 데이터셋·경량 모델(`monai_mini`)의 오차 거리 분포 형태 비교 (표 14의 2·4행).** `gaussian-tip`은 봉우리가 0–5 px 구간에 서고 전체 GT 팁의 59.4 %를 5 px 이내로 맞히는 반면, `gradient-seg`는 5 px 이내가 7.2 %뿐이고 봉우리가 5–10 px로 밀려 있다. 대신 미탐지는 `gradient-seg`가 3.47 %로 `gaussian-tip`(5.85 %)보다 낮다. 50 px 이상 구간의 합은 14.1 % 대 11.8 %로 큰 차이가 없다 — 두 방식의 차이는 오차의 *크기*가 아니라 *종류*에 있다.
...
**그림 5. `gaussian-tip`과 `gradient-seg`의 오차 비교 — 초과-임계 영역의 크기가 만드는 트레이드오프.** 위: 값이 0.5를 넘는 영역이 `gaussian-tip`에서는 팁 중심 반경 17.7 px의 원판이지만 `gradient-seg`에서는 도구 몸통 중 팁에 가까운 절반(≈ L/2)이다. 가운데: 팁으로부터의 거리에 따른 타겟값 프로파일. `gaussian-tip`의 0.5 교차점은 σ에 의해 17.7 px로 고정되는 반면, `gradient-seg`의 교차점은 도구 길이 L의 절반이라 30–80 px 범위에서 변한다. 아래: 그 결과로 나타나는 실측 지표(표 13·표 15). 정밀도는 `gaussian-tip`이, 탐지 실패율은 `gradient-seg`가 우세하며 Hit@50 px는 대등하다.
...
| `cholec80`/`gradient-seg`/`monai` | +4 / −2 px | 36,664 | 15.26 → 15.00 px | 29.55 → 29.07 % |
| `cholec80`/`gradient-seg`/`monai_mini` | +4 / −2 px | 35,886 | 15.65 → 15.03 px | 31.46 → 30.42 % |
| `cholec80`/`gaussian-tip`/`monai` | 0 / 0 px | 45,393 | 4.24 → 4.24 px | 65.38 → 65.38 % |
| `cholec80`/`gaussian-tip`/`monai_mini` | 0 / 0 px | 44,773 | 4.47 → 4.47 px | 64.68 → 64.68 % |

의미 있는 편차가 있었던 다섯 조합의 val 추정값은 기존 분석과 일치했다. `gradient-seg`의 영상별 중앙값 부호는 각 데이터셋 안에서 완전히 고정되지는 않았으므로, 도구 진입 방향이라는 가설은 지지되지만 확정되지는 않았다. 특히 `cholec80`/`gradient-seg`에서는 전체 Hit@10이 소폭 감소해, 하나의 상수 보정이 모든 장면에 이득을 주지는 않는다는 한계도 확인됐다.
...
**그림 6. `erop`/`gaussian-tip`/`monai_mini`의 매칭 유형별 오차 분해 (표 24의 4행).** GT 팁 50,718개를 단독 매칭(72.6 %)·공유 매칭(21.6 %)·미탐지(5.9 %)로 나누면, 오차 분포의 긴 꼬리가 공유 매칭 집단에 거의 전부 몰려 있음이 드러난다. 단독 매칭의 P90은 10.05 px인데 공유 매칭의 P90은 284.29 px로 28배 차이이며, 전체 평균(28.58 px)이 중앙값(3.16 px)의 9배가 되는 이유가 여기 있다. 공유 매칭은 모델이 프레임 내 도구 수보다 적은 피크를 만든 결과이므로 본질은 과소 탐지지만, 현행 평가 프로토콜은 이를 miss가 아니라 거리 오차로 집계한다.
...
| `cholec80`/`gradient-seg`/`monai` | .307 / .295 / .301 | .606 / .583 / .594 | .805 / .774 / .789 |
| `cholec80`/`gradient-seg`/`monai_mini` | .321 / .314 / .317 | .581 / .569 / .575 | .782 / .767 / .775 |
| `cholec80`/`gaussian-tip`/`monai` | .814 / .653 / .724 | .896 / .719 / .798 | .947 / .760 / .843 |
| `cholec80`/`gaussian-tip`/`monai_mini` | .812 / .646 / .720 | .894 / .711 / .792 | .945 / .752 / .838 |

50 px 상한에서 recall은 **75.2–81.8 %**로, 기존 최근접 miss율 2.1–5.9 %가 실질적인 미배정 비율을 크게 과소평가했음을 보인다. 반면 10 px에서는 `gaussian-tip`의 F1이 .720–.797로 `gradient-seg`의 .301–.454보다 높아, 좁은 거리에서의 위치 정밀도 우위는 일대일 지표에서도 유지된다.

기존 §6.7.1의 "도구 1개일 때 miss율이 가장 높다"는 패턴은 새 지표에서 사라졌다. 50 px 헝가리안 FN율은 도구 1개 프레임에서 14.0–16.5 %, 2개 프레임에서 13.5–18.9 %였고, 특정 도구 수가 모든 조합에서 부당하게 유리하거나 불리하지 않았다. 즉 이전의 3.8배 역설은 단일 피크를 여러 GT가 공유할 수 있었던 평가 산술의 산물이었다.
...
**그림 7. `run tooltip-detector` GUI 화면.** 학습된 모델로 복강경 프레임을 추론하여 원본·히트맵·예측 팁·GT·오차를 함께 시각화한다. 상단 `Dataset`·`Target`·`Model` 드롭다운으로 8개 조합을 실행 중에 전환하며 비교할 수 있고, Threshold·NMS 슬라이더로 후처리 파라미터를 실시간 조정하면서 탐색한 프레임의 누적 Hit-rate·거리 통계를 집계한다.
...
2. **시간적 누수.** 두 데이터셋 모두 영상 내부를 프레임 단위로 무작위 셔플하여 분할하므로, 같은 영상의 인접(≈0.3–1초 간격) 프레임이 train과 test에 동시에 존재한다(§2.2). 따라서 본 보고서의 수치는 **미지의 새 영상에 대한 일반화 성능이 아니다.** 영상 단위 분할(leave-video-out) 평가는 아직 수행되지 않았다. 다만 tooltip-annotator가 이 저장소에 병합되어 분할 생성 코드가 저장소 내에 있으므로, 재추출 없이 기존 프레임을 영상 단위로 재분할하는 후속 실험은 가능해졌다.
...
5. **계통 편차 보정 — 축소.** val 세트의 20 px 이내 매칭으로 편차를 추정해 8개 체크포인트에 `bias.json`으로 저장하고, test에서 원본·보정 지표를 함께 산출했다(§6.3.2). 의미 있는 편차는 기존과 같은 5개 조합에서 확인됐고, 특히 `erop`/`gaussian-tip`/`monai`의 전체 중앙값은 7.81 → 4.24 px로 개선됐다. 상수 보정이 모든 영상에서 이득을 보장하지는 않으므로, 원본 수치는 계속 표준 결과로 보존한다.
...
| 완료 (1단계) | 계통 편차 보정 (val 편차 추정 → 추론 시 차감) | 8개 조합의 원본·보정 지표 병기 (§6.3.2) |
