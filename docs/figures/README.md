...
[`docs/reports-yolo-variants.md`](../reports-yolo-variants.md)에 삽입되는 그림이다.
...
| 파일 | 종류 | 참조 위치 |
| ---- | ---- | --------- |
| `yolo-variants-detection-ap` | 데이터 | 그림 3, §3.1 클래스별 탐지 성능 |
| `yolo-variants-hit-curve` | 데이터 | 그림 4, §3.2 거리 임계값 전 구간 적중률 |
| `yolo-variants-error-distribution` | 데이터 | 그림 5, §3.2 오차 거리 분포 |
| `yolo-variants-precision-recall` | 데이터 | 그림 6, §3.3 헝가리안 정밀도·재현율 |
| `yolo-variants-convergence` | 데이터 | 그림 7, §3.4 에포크별 val mAP@0.5:0.95 |
| `yolo-variants-speed-accuracy` | 데이터 | 그림 8, §3.5 속도 대 정확도 |
| `yolo-variants-demo` | 화면 캡처 | 그림 9, §3.6 GUI 데모 실행 화면 |
| `yolo-variants-assignment` | 데이터 | 그림 10, §4.1 라벨 할당 밀도 |

그림 1·2(각 모델 구조)는 이 폴더가 아니라 각 서브 프로젝트에 있고, 보고서가 그리로 링크한다:
`baseline/cladnet/docs/images/model-architecture.png`,
`baseline/yolov8sclone/docs/images/model-architecture.png`.

그림 10은 파일이 아니라 두 모델의 라벨 할당기를 실제로 한 번 돌려 만든다(학습은 하지 않는다).
서브 프로젝트마다 가상환경이 다르므로 노트북이 각 프로젝트의 인터프리터를 따로 호출한다.

그림 9는 노트북이 만들지 않는다. `DISPLAY`가 있는 환경에서 `./baseline/yolov8sclone/run demo`를
