#!/usr/bin/env python3
"""GUI demo for the YOLO26-clone surgical-tool baseline.

Plays a laparoscopic video -- or a directory of extracted dataset frames --
through a trained checkpoint (`baseline/yolo26clone/data/model/<dataset>/model.pt`,
...
--------
  ...
  Conf                   detection confidence threshold. There is no IoU
                         control: this head is end-to-end, so it emits one box
                         per object and no NMS runs at all
  Show GT                overlay ground truth (dataset frame sources only)

Usage
-----
    ./baseline/yolo26clone/run demo
    ./baseline/yolo26clone/run demo --dataset erop
    ./baseline/yolo26clone/run demo --device cpu
"""
...
# NOTE: deliberately not a top-level import. `common.*` needs baseline/yolo26clone
...
class Yolo26CloneDemoApp(tk.Tk):
    def __init__(self, weights: str, device: str | None,
                 videos_root: str, frames_root: str):
        super().__init__()
        self.title("YOLO26 clone — Surgical Tool & Tip Detection Demo")
    ...
    def _build_ui(self):
        ...
        # No IoU slider: this head is end-to-end, so nothing runs NMS and an
        # IoU threshold would be a control that does nothing.
