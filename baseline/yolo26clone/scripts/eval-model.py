#!/usr/bin/env python3
"""Evaluate a trained YOLO26-clone checkpoint on a dataset split.
...
Outputs, under baseline/yolo26clone/data/results/<dataset>/<split>/ by default:
...
Usage:
    ...
    ./baseline/yolo26clone/run eval-model --dataset cholec80 --split val --limit 2000
"""
...
if True:
    ...
    from common.dataset import (CLASS_NAMES, SPLITS, TIP_CLASS, TOOL_CLASS,
                                ...
                                default_data_root)
    from common.inference import (DEFAULT_CONF, DEFAULT_MAX_DET, Detector, results_dir,
                                  default_model_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate the YOLO26 clone on a tooltip-detector dataset")
    ...
    parser.add_argument("--max-det", type=int, default=DEFAULT_MAX_DET,
                        help=f"cap on boxes per frame. YOLO26's head is end-to-end, so this "
                             f"replaces the NMS IoU threshold (default: {DEFAULT_MAX_DET})")
    ...
    for index in progress(range(len(dataset)), desc="eval"):
        frame, labels = dataset.read_frame(index)
        height, width = frame.shape[:2]
        session = dataset.session_id(index)
        frame_name = dataset.frame_name(index)
