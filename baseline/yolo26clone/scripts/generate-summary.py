...
# directories. --suffix exists for the comparison rounds the README describes:
# a run with --optimizer sgd, or a different tip box size, kept side by side
# under data/model<suffix>/ and summarised on its own.
DATA_SUFFIX = ""
...
# and its third term is a plain L1. The values reported are the one-to-one
# branch's, the branch inference reads.
...
def section_settings(out: list[str], records: list[dict]) -> None:
    ...
    keys = ("epochs", "batch_size", "optimizer", "lr", "momentum", "weight_decay",
            "image_size", "tip_box_size", "scale", "frame_stride", "val_frames", "no_ema",
            "conf", "max_det", "device", "workers")
...
def section_convergence(out: list[str], records: list[dict]) -> None:
    ...
    for record in records:
        ...
        converged = ("마지막 에포크가 최고점이다. **수렴하지 않았다**" if best == len(curve)
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarise YOLO26-clone training and evaluation artefacts into a markdown document")
