...
can pick for it. This baseline defaults to 32 px, the same as the YOLOv8s clone
next door, where the CLAD-Net baseline uses 10 px -- the three are *not*
interchangeable without saying which size was used, so `--tip-box-size` records
...
# centred on the point, so it can be learned by a box detector. YOLO26 assigns
# labels dynamically from anchor points *inside* the box, so a larger box gives
# the tip class more candidate positives than CLAD-Net's 10 px box does. The
# assigner does floor a box's sides at one stride (see common/assigner.py),
# which softens but does not remove the effect.
