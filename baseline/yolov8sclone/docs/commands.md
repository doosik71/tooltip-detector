...
데이터셋마다 한 번씩, 산출물은 `baseline/yolov8sclone/data/model/<dataset>/<label-set>/`에
...
```bash
./baseline/yolov8sclone/run train-model --dataset cholec80 --label-set tiponly --frame-stride 5 --val-frames 1500 --workers 12
./baseline/yolov8sclone/run train-model --dataset erop     --label-set tiponly --frame-stride 5 --val-frames 1500 --workers 12
```
