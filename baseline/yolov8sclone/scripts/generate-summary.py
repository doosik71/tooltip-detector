...
Usage:
    ...
    ./baseline/yolov8sclone/run generate-summary --suffix "" --output docs/summary-results.md
"""
...
# Loss columns differ between the two baselines; YOLOv8 has no objectness term
# and carries a Distribution Focal Loss instead.
...
def section_settings(out: list[str], records: list[dict]) -> None:
    ...
    keys = ("epochs", "batch_size", "lr", "momentum", "weight_decay", "image_size",
            "tip_box_size", "scale", "frame_stride", "val_frames", "no_ema",
            "conf", "iou", "device", "workers")
...
def section_confidence(out: list[str], records: list[dict]) -> None:
    ...
    out += table(["데이터셋", "표본", "n", "p10", "중앙값", "p90", "최대", "≥ 0.25"],
                 rows, "ll" + "r" * 6)
    out += ["> YOLOv8에는 objectness 항이 없고 클래스 점수가 곧 신뢰도다. 그 학습 목표는",
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarise YOLOv8s-clone training and evaluation artefacts into a markdown document")
