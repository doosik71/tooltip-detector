...
class Detector:
    ...
    def __init__(self, weights: str, device: str | None = None):
        ...
        # fifth of the frame time, and it is what the reference runs at
        # inference too (`AutoBackend` fuses on load). It has to come after
        # `load_state_dict`, since folding rewrites `state_dict` keys; the
        # checkpoint format on disk and the training path are untouched.
        #
    ...
    def detect(self, frame_rgb: np.ndarray, conf: float = DEFAULT_CONF,
        ...
        predictions = decode(self.model(tensor), self.model)
