#!/usr/bin/env python3
"""GUI demo for the YOLO26 surgical-tool baseline.

Plays a laparoscopic video -- or a directory of extracted dataset frames --
through a trained checkpoint (`baseline/yolo26/data/model/<dataset>/model.pt`,
...
--------
  ...
  Conf                   detection confidence threshold. There is no IoU
                         control: YOLO26's head is end-to-end, so it emits one
                         box per object and no NMS runs at all
  Show GT                overlay ground truth (dataset frame sources only)

Usage
-----
    ./baseline/yolo26/run demo
    ./baseline/yolo26/run demo --dataset erop
    ./baseline/yolo26/run demo --device cpu
"""
...
# NOTE: deliberately not a top-level import. `common.*` needs baseline/yolo26
...
class Yolo26DemoApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # No IoU slider: YOLO26's head is end-to-end, so predict() never runs
        # NMS and an IoU threshold would be a control that does nothing.
        tk.Label(param, text="(end-to-end head: no NMS, no IoU threshold)",
                 fg="gray").pack(side=tk.LEFT, padx=(0, 12))
    ...
    def _render(self, frame: np.ndarray, index: int):
        ...
        detections, infer_ms = self._detector.detect(
            frame, conf=self._conf_var.get(), max_det=DEFAULT_MAX_DET)
...
def main():
    parser = argparse.ArgumentParser(
        description="GUI demo for the YOLO26 surgical tool + tip detection baseline")
    ...
    app = Yolo26DemoApp(weights=weights, device=args.device,
