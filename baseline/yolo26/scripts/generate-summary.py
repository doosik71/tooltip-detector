...
Usage:
    ...
    ./baseline/yolo26/run generate-summary --suffix "-tip16" --output docs/summary-tip16.md
"""
...
def section_settings(out: list[str], records: list[dict]) -> None:
    ...
    out += ["> `optimizer=auto`는 이 규모의 학습에서 MuSGD로 해석되며, 그때 `lr`은 무시되고",
...
def section_detection(out: list[str], records: list[dict]) -> None:
    ...
    if first:
        out += [f"평가 조건: `conf={first['conf_threshold']}`, `max_det={first.get('max_det', '—')}`, "
...
def section_confidence(out: list[str], records: list[dict]) -> None:
    ...
    out += table(["데이터셋", "표본", "n", "p10", "중앙값", "p90", "최대", "≥ 0.25"],
                 rows, "ll" + "r" * 6)
    out += ["> YOLO26에도 objectness 항이 없고 클래스 점수가 곧 신뢰도다. 게다가 헤드가",
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarise YOLO26 training and evaluation artefacts into a markdown document")
