...
train-model / eval-model write:
  ...
  data/model<SUFFIX>/<dataset>/
      ...
      metric.csv          one row per epoch (losses, mAP, per-class AP, lr, time),
                          mirrored out of Ultralytics' results.csv by train-model
...
`--suffix=-scratch`, not `--suffix "-scratch"`: the value starts with a dash, so
argparse reads the separated form as another flag and fails.
"""
...
if True:
    ...
    from common.dataset import data_dir

# There is only one round of this baseline so far, and it writes to the
# unsuffixed directories. --suffix exists for the same reason it does in the
# sibling baselines: a comparison round (a different tip box size, say) can be
# kept side by side under data/model<suffix>/ and summarised on its own.
DATA_SUFFIX = ""
...
def parameter_count(records: list[dict]) -> int | None:
    """Counted off a trained checkpoint: no artefact on disk records it.

    Loading it needs `ultralytics`, which is a several-second import, so this
    is done once for the first checkpoint found and skipped entirely when
    nothing has been trained yet.
    """
    for record in records:
        weights = os.path.join(record["model_dir"], "model.pt")
...
def section_overview(out: list[str], records: list[dict], gaps: list[str],
    ...
    for record in records:
        ...
        rows.append([
            ...
            "—" if status is None else num(status.get("best_fitness")),
            "있음" if summary is not None else "**없음**",
            "있음" if record["per_tip"] is not None else "**없음**",
        ])
    out += table(["데이터셋", "학습 에포크", "best val fitness", "평가 결과", "per_tip.csv"],
                 rows, "lrrll")

    params = parameter_count(records)
...
def section_settings(out: list[str], records: list[dict]) -> None:
    ...
    keys = ("scale", "epochs", "batch_size", "image_size", "optimizer", "lr",
            "patience", "no_pretrained", "no_amp", "device", "workers")
    info_keys = ("tip_box_size", "frame_stride", "pretrained")
    ...
    for key in info_keys:
        cells = []
        for record in records:
            info = (record["status"] or {}).get("info", {})
            cells.append("—" if key not in info else str(info[key]))
        if all(c == "—" for c in cells):
            continue
        rows.append([f"`{key}`"] + cells)
    out += table(headers, rows, "l" + "l" * len(records))
    out += ["> 기본 옵티마이저는 `SGD`다. Ultralytics 8.4의 `optimizer=auto`는 모델과 무관하게",
...
def section_detection(out: list[str], records: list[dict]) -> None:
    ...
    if first:
        out += [f"평가 조건: `conf={first['conf_threshold']}`, "
                f"NMS `iou={first.get('iou_threshold', '—')}`, "
                f"`max_det={first.get('max_det', '—')}`, "
...
def section_confidence(out: list[str], records: list[dict]) -> None:
    ...
    out += table(["데이터셋", "표본", "n", "p10", "중앙값", "p90", "최대", "≥ 0.25"],
                 rows, "ll" + "r" * 6)
    out += ["> YOLOv8에는 objectness 항이 없고 클래스 점수가 곧 신뢰도다. 그 목표값은",
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarise YOLOv8s training and evaluation artefacts into a markdown document")
    parser.add_argument("--suffix", default=DATA_SUFFIX,
                        help=f'directory suffix under data/ (default: "{DATA_SUFFIX}", the '
                             'unsuffixed directories train-model and eval-model write to). '
                             'Pass it joined by "=", as in --suffix=-scratch: the value '
                             'starts with a dash')
