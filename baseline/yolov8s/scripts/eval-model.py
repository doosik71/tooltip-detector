#!/usr/bin/env python3
"""Evaluate a trained YOLOv8s checkpoint on a dataset split.
...
CLAD-Net, YOLOv8s-clone and YOLO26 baselines report these exact definitions,
and a number is only comparable if it was produced the same way. Frames and
...
Unlike YOLO26 next door, YOLOv8's head is not end-to-end, so NMS runs and
`--iou` is a real parameter. Its default (0.45) is the YOLOv8s-clone
baseline's rather than Ultralytics' 0.7, so the two report the same operating
point.

Everything is measured in original frame pixels (736 x 480), not in the
letterboxed 640 x 640 network input.

Outputs, under baseline/yolov8s/data/results/<dataset>/<split>/ by default:
...
Note: this evaluates the 2-class `tool`/`tip` model produced by
scripts/train-model.py, not the published 7-class checkpoint the demo uses.
That one has no `tip` class and no class labels to score against, so these
metrics are undefined for it.

Usage:
    ./baseline/yolov8s/run eval-model --dataset cholec80
    ./baseline/yolov8s/run eval-model --dataset cholec80 --split val --limit 2000
"""
...
if True:
    ...
    from common.dataset import (SPLITS, SplitFrames, available_datasets,
                                default_data_root, results_dir)
    from common.inference import (DEFAULT_CONF, DEFAULT_IOU, DEFAULT_MAX_DET, Detector,
                                  class_index, default_model_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate YOLOv8s on a tooltip-detector dataset")
    ...
    parser.add_argument("--iou", type=float, default=DEFAULT_IOU,
                        help=f"NMS IoU threshold (default: {DEFAULT_IOU}, matching "
                             "baseline/yolov8sclone rather than Ultralytics' 0.7)")
    ...
    parser.add_argument("--max-det", type=int, default=DEFAULT_MAX_DET,
                        help="cap on boxes per frame, applied after NMS")
    ...
    if "tip" not in detector.class_names:
        raise SystemExit(
            f"this checkpoint has no `tip` class (classes: {', '.join(detector.class_names)}).\n"
            "The tip metrics are undefined for it -- train a 2-class model with "
            "scripts/train-model.py, or use scripts/demo.py to look at this one.")
    if detector.end2end:
        print("note    : this checkpoint has an end-to-end head; --iou has no effect on it")
    ...
    for index in progress(range(len(frames)), desc="eval"):
        ...
        detections, elapsed_ms = detector.detect(frame, args.map_conf, args.iou,
                                                 args.max_det)
        total_ms += elapsed_ms
        n_frames += 1
