#!/usr/bin/env python3
"""Train YOLOv8s on this repository's surgical-tool annotations.
...
Why this run exists at all, given that baseline/yolov8sclone already trains
YOLOv8s: the clone is a reimplementation *without* Ultralytics, and a
reimplementation needs a reference to be read against. This is that reference,
the same way baseline/yolo26 is the reference for baseline/yolo26clone. The
architecture is identical down to the parameter count (11,136,374 at 2
classes), so a gap between the two is a gap in the recipe, not in the model.

Two defaults are chosen for that comparison rather than copied from
Ultralytics:

  --optimizer SGD   Ultralytics 8.4's `auto` picks MuSGD on a run this long,
                    for any model -- it is not YOLO26-specific. MuSGD is not
                    the optimizer YOLOv8 was published with and not the one
                    yolov8sclone uses, so it is not the default here. Pass
                    `--optimizer auto` to get it.
  --lr 0.01         YOLOv8's published initial rate, and the clone's.

Starting point: `yolov8<scale>.pt`, the COCO-pretrained weights, downloaded
once into data/pretrained/. `--no-pretrained` starts from random
initialisation instead, which is what the CLAD-Net and YOLOv8s-clone baselines
do -- use it when the comparison against those two has to be like-for-like.

Usage:
    ./baseline/yolov8s/run train-model --dataset cholec80
    ./baseline/yolov8s/run train-model --dataset erop --device cuda:1 --no-pretrained
"""
...
def pretrained_weights(scale: str) -> str:
    ...
    Not to be confused with `data/yolov8s_cholec80.pt`, which
    scripts/download-model.py fetches: that one is a published 7-class
    instrument classifier used by the demo, and has nothing to do with the
    `tool`/`tip` model trained here.
    """
    from ultralytics.utils.downloads import attempt_download_asset

    target = os.path.join(data_dir(), "pretrained", f"yolov8{scale}.pt")
    if not os.path.exists(target):
        os.makedirs(os.path.dirname(target), exist_ok=True)
        print(f"downloading yolov8{scale}.pt -> {target}")
...
def metric_row(trainer) -> dict:
    ...
    The loss names are read from the trainer rather than hard-coded: YOLOv8's
    third term is `dfl_loss` where YOLO26's is `l1_loss` (that model dropped
    DFL), so the columns differ between the two baselines even though this
    function does not.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Train YOLOv8s on a tooltip-detector dataset")
    ...
    parser.add_argument("--scale", default=DEFAULT_SCALE, choices=SCALES,
                        help=f"YOLOv8 depth/width scale (default: {DEFAULT_SCALE}, the size "
                             "the sibling baselines use)")
    parser.add_argument("--epochs", type=int, default=150)
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--image-size", type=int, default=640)
    parser.add_argument("--lr", type=float, default=0.01,
                        help="initial learning rate (default: 0.01, YOLOv8's published "
                             "value); ignored when --optimizer is auto, which picks the "
                             "rate along with the optimizer")
    parser.add_argument("--optimizer", default=DEFAULT_OPTIMIZER,
                        help=f"SGD (default: {DEFAULT_OPTIMIZER}, YOLOv8's published recipe "
                             "and the one yolov8sclone uses), AdamW, MuSGD, or auto -- note "
                             "that Ultralytics 8.4's auto resolves to MuSGD on a run this "
                             "long, which is not what YOLOv8 was published with")
    ...
    parser.add_argument("--no-pretrained", action="store_true",
                        help="start from random weights instead of the COCO checkpoint; "
                             "this is the condition yolov8sclone and cladnet train under")
    parser.add_argument("--no-amp", action="store_true",
                        help="disable mixed precision (also skips Ultralytics' AMP check, "
                             "which downloads a small checkpoint the first time)")
    ...
    if resume:
        ...
        model = YOLO(f"yolov8{args.scale}.yaml")
        print(f"starting yolov8{args.scale} from random weights")
    else:
    ...
    print(f"next  ./baseline/yolov8s/run eval-model --dataset {args.dataset}")
