...
derived from that same annotation -- the same two the CLAD-Net, YOLOv8s-clone and
YOLO26 baselines use, so all of them are comparable. The clone matters most
here: it reimplements this exact architecture without Ultralytics, and this
baseline is the reference its numbers are read against.
...
def repo_root() -> str:
    here = os.path.dirname(os.path.abspath(__file__))        # baseline/yolov8s/common
    return os.path.abspath(os.path.join(here, "..", "..", ".."))


def project_root() -> str:
    """This sub-project's own directory, baseline/yolov8s/."""
...
def data_dir() -> str:
    """This baseline's own outputs, all under baseline/yolov8s/data/."""
...
def require_prepared(dataset: str) -> dict:
    ...
    if status is None or not os.path.exists(dataset_yaml_path(dataset)):
        raise SystemExit(
            ...
            f"run:  ./baseline/yolov8s/run prepare-dataset --dataset {dataset}")
    return status
