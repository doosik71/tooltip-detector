"""Loading a trained YOLOv8s checkpoint and running it on one frame.
...
Unlike YOLO26 next door, YOLOv8's head is *not* end-to-end: it emits many
overlapping boxes per object and NMS is what reduces them to one. So `detect()`
takes an IoU threshold as well as a confidence threshold, and `--iou` is a real
knob here. The default 0.45 is the YOLOv8s-clone baseline's, not Ultralytics'
0.7, because that clone is what these numbers get compared against.

This module is separate from `common/detector.py`, which wraps the published
7-class Hugging Face checkpoint for the demo. That one classifies instruments;
this one is the 2-class `tool`/`tip` model `scripts/train-model.py` produces.
"""
...
class Detector:
    ...
    def __init__(self, weights: str, device: str | None = None):
        ...
        # False for every YOLOv8 checkpoint; read rather than assumed so a
        # checkpoint from another family still reports itself correctly.
    ...
    def detect(self, frame_rgb: np.ndarray, conf: float = DEFAULT_CONF,
               iou: float = DEFAULT_IOU, max_det: int = DEFAULT_MAX_DET
        ...
        array handed in -- no rescaling by the caller. The elapsed time covers
        the whole call, NMS included, because that is what a frame costs.
        """
        ...
        result = self.model.predict(frame_bgr, conf=conf, iou=iou, max_det=max_det,
