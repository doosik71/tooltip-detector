...
class EfficientNetB2MiniEncoder(nn.Module):
    """EfficientNet-B2 encoder with halved block counts per stage.

    Channel widths are identical to the full model; only depth is reduced.

    Block counts per stage (full → mini):
      [2, 3, 3, 4, 4, 5, 2]  →  [1, 2, 2, 2, 2, 3, 1]  (23 → 13 blocks)
    """
...
class UNetDecoder(nn.Module):
    """U-Net decoder that consumes EfficientNetB2Mini feature maps.

    Skip connections are identical to the full model (same channel counts):
...
class TooltipDetector(nn.Module):
    """Surgical tool segmentation: EfficientNet-B2 Mini encoder + U-Net decoder.

    Encoder block counts halved vs the full monai model:
      [2, 3, 3, 4, 4, 5, 2] → [1, 2, 2, 2, 2, 3, 1]  (23 → 13 blocks)
    Channel widths and decoder structure are unchanged.
    """

    def __init__(self, num_classes=2):
        super().__init__()
        self.encoder = EfficientNetB2MiniEncoder()
