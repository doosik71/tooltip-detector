#!/usr/bin/env python3
"""GUI demo for the yolov8sclone tool/tip TensorRT engine on Jetson.

Plays a laparoscopic video -- or a directory of extracted dataset frames --
through a TensorRT engine built by `scripts/convert-model.py`
(`data/model/<dataset>/<label-set>/model.engine`, picked with --dataset, the
alphabetically first converted one by default) and draws what it predicts on
every frame:

  tool   the instrument's bounding box
  tip    a small square box on the instrument tip (32 px by default, whatever
         the source checkpoint was trained with), marked with a cross at its
         centre, which is the coordinate the tip metrics are computed from

This is a straight adaptation of `baseline/yolov8sclone/scripts/demo.py` (same
GUI layout, same controls) and, for the window chrome and layout conventions,
`baseline/yolov8s/scripts/demo.py`. What differs is only what runs inference:
`common.inference.Detector` here loads a TensorRT engine instead of a `.pt`
checkpoint, but returns predictions in the exact same shape, so the rendering
code below is unchanged.

Sources
-------
The "Source" dropdown is filled by scanning two places (see common/sources.py):

  data/dataset/<dataset>/images/<split>/                 736x480 frames + annotations
  <tooltip-annotator>/data/dataset-src/<dataset>/*.mp4   original videos, no labels (if mounted)
...
Usage
-----
    ./standalone/jetson/yolov8s/run demo
    ./standalone/jetson/yolov8s/run demo --dataset erop
"""
...
if True:
    ...
    from common.inference import (DEFAULT_CONF, DEFAULT_IOU, DEFAULT_LABEL_SET,
                                  Detector, default_model_path, trained_datasets)
...
class TensorRTDemoApp(tk.Tk):
    def __init__(self, weights: str, device: str | None,
                videos_root: str, frames_root: str):
        super().__init__()
        self.title("yolov8sclone (TensorRT) — Surgical Tool & Tip Detection Demo")
    ...
    def _load_detector(self, weights: str, device: str | None) -> Detector | None:
        if not os.path.exists(weights):
            print(f"WARNING: engine not found: {weights}\n"
                  "         run scripts/convert-model.py first")
            return None
        try:
            detector = Detector(weights, device=device)
            print(f"Engine loaded: {weights}  [{detector.device}]  "
    ...
    def _refresh_status(self):
        if self._detector is None:
            self._status_var.set(f"engine NOT loaded: {os.path.basename(self._weights)}")
    ...
    def _render(self, frame: np.ndarray, index: int):
        if self._detector is None:
            ...
            self._set_info(
                ...
                "no engine loaded — showing the raw frame.\n"
                "Run scripts/convert-model.py, then restart the demo.")
            return
...
def main():
    parser = argparse.ArgumentParser(
        description="GUI demo for the yolov8sclone tool + tip TensorRT engine on Jetson")
    parser.add_argument("--dataset", default=None,
                        help="load data/model/<dataset>/<label-set>/model.engine (converted: "
    ...
    parser.add_argument("--label-set", default=DEFAULT_LABEL_SET, choices=LABEL_SETS,
                        help="which converted engine to open: `tooltip` learned the tool box "
                             "and the tip box, `tiponly` the tip box alone "
                             f"(default: {DEFAULT_LABEL_SET})")
    parser.add_argument("--weights", default=None,
                        help="engine path, overriding --dataset and --label-set "
                             f"(default: {default_model_path()})")
    parser.add_argument("--device", default="cuda:0",
                        help="CUDA device the engine was built for (default: cuda:0)")
    ...
    app = TensorRTDemoApp(weights=weights, device=args.device,
