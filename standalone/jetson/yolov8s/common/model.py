...
Verbatim copy of `baseline/yolov8sclone/common/model.py`. It has to stay
byte-identical to that file: `scripts/convert-model.py` loads
`model.pt` checkpoints produced by the original module, and `load_state_dict`
only succeeds if this module builds the exact same parameter names and
shapes. If the source module changes, re-copy it here rather than editing
by hand.
